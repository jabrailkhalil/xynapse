The `extensions/vscode` folder contains code for the "Xynapse" VS Code Extension. Xynapse Assistant is a coding extension which extends IDE functionality with AI.
...
Shared code that has abstract logic for the extension is in `core/`. Avoid importing code from `core/` directly where possible. Core is designed to be bundlable as a binary. Use `core.invoke` to send messages to the
