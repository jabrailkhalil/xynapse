...
function buildWin32Setup(arch: string, target: string): task.CallbackTask {
	...
	return (cb) => {
		...
		const hasAppxContextMenu = Boolean((product as typeof product & { win32ContextMenu?: Record<string, { clsid: string }> }).win32ContextMenu?.[arch]);
		if ((quality === 'stable' || quality === 'insider') && hasAppxContextMenu) {
