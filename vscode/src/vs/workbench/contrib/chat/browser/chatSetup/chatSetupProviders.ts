...
import { ChatEntitlementContext, ChatEntitlementRequests, IChatEntitlementService } from '../../../../services/chat/common/chatEntitlementService.js';
...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	private async doInvoke(request: IChatAgentRequest, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatWidgetService: IChatWidgetService, chatAgentService: IChatAgentService, languageModelToolsService: ILanguageModelToolsService): Promise<IChatAgentResult> {
		// Check for workspace trust first
		if (this.context.state.untrusted) {
			progress({
		...
		// Xynapse: Show Copilot setup dialog when needed
		// If Copilot is not installed/registered, show setup dialog
		if (!this.context.state.installed || !this.context.state.registered) {
			// Try to invoke with setup for Copilot login
			// Message will be shown by doInvokeWithSetup if user cancels
		...
		// Copilot is set up, forward directly
	...
	private async forwardRequestToChat(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			...
			// Xynapse: More detailed error logging
			this.logService.error('[chat setup] Failed to forward request to chat:', error);
			this.logService.error('[chat setup] Error details:', {
	...
	private whenLanguageModelReady(languageModelsService: ILanguageModelsService, modelId: string | undefined): Promise<unknown> | void {
		const hasModelForRequest = () => {
			...
			// Xynapse: Accept ANY available model, not just default ones
			// This allows YandexGPT and other local providers to work
			const modelIds = languageModelsService.getLanguageModelIds();
			if (modelIds.length > 0) {
				return true; // We have at least one model available
			}
	...
	private whenToolsModelReady(languageModelToolsService: ILanguageModelToolsService, requestModel: IChatRequestModel): Promise<unknown> | void {
		...
		// Xynapse: Accept ANY tools, not just copilot_ ones
		// This allows local language model providers to work
		const hasAnyTools = () => {
			for (const _tool of languageModelToolsService.getTools()) {
				return true; // we have at least one tool
			}
			return false;
		};
		...
		return Event.toPromise(Event.filter(languageModelToolsService.onDidChangeTools, () => hasAnyTools()));
	}

	private whenAgentReady(chatAgentService: IChatAgentService, mode: ChatModeKind | undefined): Promise<unknown> | void {
		// Xynapse: Accept ANY agent including core ones
		// This allows local language model providers to work without external agents
		const defaultAgent = chatAgentService.getDefaultAgent(this.location, mode);
		if (defaultAgent) {
			return; // we have any default agent - good enough!
		}
	...
	// Xynapse: Method kept but bypassed - local language model providers work directly
