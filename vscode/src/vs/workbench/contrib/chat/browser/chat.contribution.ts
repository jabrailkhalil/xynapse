...
import { ChatExtensionPointHandler } from './chatParticipant.contribution.js';
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[PromptsConfig.USE_AGENT_SKILLS]: {
			...
			markdownDescription: nls.localize('chat.useAgentSkills.description', "Controls whether skills are provided as specialized capabilities to the chat requests. Skills are loaded from `.github/skills`, `~/.copilot/skills`, `.xynapse/skills`, and `~/.xynapse/skills`. The language model can load these skills on-demand if the `read` tool is available. Learn more about [Agent Skills](https://aka.ms/vscode-agent-skills).",),
			default: false,
			restricted: true,
			disallowConfigurationDefault: true,
			tags: ['experimental', 'prompts', 'reusable prompts', 'prompt snippets', 'instructions']
...
Registry.as<IConfigurationMigrationRegistry>(Extensions.ConfigurationMigration).registerConfigurationMigrations([
	...
	{
		key: 'chat.useXynapseSkills',
		migrateFn: (value, _accessor) => ([
			['chat.useXynapseSkills', { value: undefined }],
...
// Copilot UI disabled — Xynapse Assistant replaces it
