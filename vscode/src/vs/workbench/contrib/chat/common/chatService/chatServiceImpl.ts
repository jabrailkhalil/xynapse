...
export class ChatService extends Disposable implements IChatService {
	...
	async activateDefaultAgent(location: ChatAgentLocation): Promise<void> {
		...
		if (!defaultAgentData) {
			// Xynapse: Don't throw error when no external agent is contributed
			// This allows local language model providers (like YandexGPT) to work
			this.logService.debug('[chat] No default agent contributed, using core agent');
			return;
		}
		...
		if (!defaultAgent) {
			// Xynapse: Don't throw error, just log warning
			this.logService.warn('[chat] Default agent not activated, but continuing with core functionality');
			return;
		}
	}
	...
	async resendRequest(request: IChatRequestModel, options?: IChatSendRequestOptions): Promise<void> {
		...
		// Xynapse: Handle case when no default agent is available
		if (!defaultAgent) {
			this.logService.warn('[chat] No default agent available for location:', location);
			throw new Error('No chat agent available. Please configure a language model provider.');
		}
	...
	async sendRequest(sessionResource: URI, request: string, options?: IChatSendRequestOptions): Promise<IChatSendRequestData | undefined> {
		...
		// Xynapse: Handle case when no default agent is available
		if (!defaultAgent) {
			this.logService.warn('[chat] No default agent available for sendRequest, location:', location);
			return undefined;
		}
