...
export function CliInstallBanner({
  ...
  const { copyText } = useCopy("npm i -g @xynapse/cli");
  ...
  useEffect(() => {
    ...
    const checkCliInstallation = async () => {
      try {
        ...
        // Use non-throwing checks to avoid noisy command-failed logs.
        const command =
          platform === "windows"
            ? 'powershell -NoProfile -Command "if (Get-Command cn -ErrorAction SilentlyContinue) { (Get-Command cn).Source }"'
            : "command -v cn || true";

        const [stdout] = await ideMessenger.ide.subprocess(command);
          ...
          <div className="flex flex-col items-start gap-2 self-stretch">
            <div className="rounded-default outline-command-border flex items-center self-stretch outline outline-1">
              ...
              <div className="bg-background rounded-r-default flex items-center gap-2 px-3 py-3">
                <CopyButton
                  text={`npm i -g @xynapse/cli && cn "Explore this repo and provide a concise summary of it's contents"`}
                />
                <RunInTerminalButton
