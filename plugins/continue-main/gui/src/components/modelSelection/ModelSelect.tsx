...
function isMissingApiKeyModel(model: any): boolean {
  return model?.apiKey === "";
}
...
function ModelSelect() {
  ...
  let allModels = null;
  let activeRole: ModelRole = isInEdit ? "edit" : "chat";
  if (isInEdit) {
  ...
  if ((!allModels || allModels.length === 0) && config.modelsByRole) {
    const fallbackRoleOrder = [
      "chat",
      "edit",
      "apply",
      "summarize",
    ...
    for (const role of fallbackRoleOrder) {
      const modelsForRole = config.modelsByRole[role]?.filter(
        (model: any) => !isMissingApiKeyModel(model),
      );
      if (!modelsForRole || modelsForRole.length === 0) {
        continue;
      }
      allModels = modelsForRole;
      activeRole = role;
      if (!selectedModel) {
        selectedModel = config.selectedModelByRole[role] ?? modelsForRole[0];
      }
      break;
    }
  }
  if (isMissingApiKeyModel(selectedModel)) {
    selectedModel = null;
  }
  ...
  useEffect(() => {
    setOptions(
      (allModels ?? [])
        .filter((model: any) => !isMissingApiKeyModel(model))
        .map((model: any) => {
          return {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      if (
        ...
        const currentIndex = options.findIndex(
          (option) => option.value === modelOptionValue(selectedModel),
        );
  ...
  }, [activeRole, dispatch, options, selectedModel, selectedProfile]);

  function onClickAddModel(e: React.MouseEvent) {
    e.stopPropagation();
    e.preventDefault();
  ...
  return (
    <Listbox
      onChange={async (val: string) => {
        ...
        if (val === modelOptionValue(selectedModel)) return;
        void dispatch(
          updateSelectedModelByRole({
            selectedProfile,
            role: activeRole,
            ...
            {isConfigLoading ? (
              ...
              sortedOptions.map((option, idx) => (
                <ModelOption
                  ...
                  isSelected={option.value === modelOptionValue(selectedModel)}
                />
              ))
            )}
          </div>
