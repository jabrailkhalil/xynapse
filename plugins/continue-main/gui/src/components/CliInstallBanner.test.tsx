...
describe("CliInstallBanner", () => {
  ...
  describe("CLI detection", () => {
    it("does not render when CLI is installed (subprocess returns path)", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.queryByText("Try out the Xynapse CLI"),
    ...
    it("renders when CLI is not installed (subprocess returns empty)", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Try out the Xynapse CLI")).toBeInTheDocument();
      });
    });
    ...
    it("uses non-throwing command check on mac platform", async () => {
      vi.mocked(util.getPlatform).mockReturnValue("mac");
      ...
      await waitFor(() => {
        expect(subprocessSpy).toHaveBeenCalledWith("command -v cn || true");
      });
    });

    it("uses non-throwing command check on linux platform", async () => {
    ...
    it("uses non-throwing command check on windows platform", async () => {
      ...
      await waitFor(() => {
        expect(subprocessSpy).toHaveBeenCalledWith(
          'powershell -NoProfile -Command "if (Get-Command cn -ErrorAction SilentlyContinue) { (Get-Command cn).Source }"',
        );
      });
    });
  ...
  describe("Banner content", () => {
    ...
    it("displays the installation command", () => {
      expect(screen.getByText("npm i -g @xynapse/cli")).toBeInTheDocument();
    });
    ...
    it("displays the CommandLine icon", () => {
      ...
      const banner = screen.getByText("Try out the Xynapse CLI").closest("div");
  ...
  describe("User interactions", () => {
    ...
    it("opens documentation URL when Learn more link is clicked", async () => {
      ...
      expect(postSpy).toHaveBeenCalledWith(
        "openUrl",
        "https://docs.xynapse.dev/guides/cli",
      );
    });
    ...
    it("runs installation command in terminal when run button is clicked", async () => {
      ...
      if (runButton) {
        ...
        expect(postSpy).toHaveBeenCalledWith("runCommand", {
          command: `npm i -g @xynapse/cli && cn "Explore this repo and provide a concise summary of it's contents"`,
        });
      }
    });
  });
