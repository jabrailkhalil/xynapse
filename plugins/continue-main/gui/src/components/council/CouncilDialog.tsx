...
import { loadBVCParams, type BVCParams } from "../bvc/BVCSettingsDialog";
...
const COUNCIL_DEFAULT_ROLES: string[] = [
  "PM",
  "Architect",
  "Developer",
  "Reviewer",
...
const BVC_DEFAULT_ROLES: string[] = [
  "Architect",
  "Developer",
  "Reviewer",
  "Tester",
...
function CouncilDialog({ mode = "council", onClose, onSubmit }: CouncilDialogProps) {
  ...
  const allModels = useMemo(() => {
    ...
    const add = (m: unknown) => {
      const model = m as ModelDescription | undefined;
      const key = model?.title || model?.model;
      if (key && !seen.has(key)) {
        seen.add(key);
        models.push(model);
      }
    ...
    if (Array.isArray((config as any).models)) {
      (config as any).models.forEach(add);
    }
    Object.values(config.selectedModelByRole ?? {}).forEach(add);
  ...
  const [roles, setRoles] = useState<CouncilRoleConfig[]>(
    (isBvc ? BVC_DEFAULT_ROLES : COUNCIL_DEFAULT_ROLES).map((name) => ({
      name,
      modelTitle: defaultModelTitle,
    })),
  );
  ...
  const handleSubmit = () => {
    if (!task.trim()) return;
    const bvcParams = isBvc ? loadBVCParams() : undefined;
    onSubmit({
      difficulty,
      task: task.trim(),
      roles,
