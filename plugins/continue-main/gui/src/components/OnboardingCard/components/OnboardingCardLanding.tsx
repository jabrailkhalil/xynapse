...
import XynapseLogo from "../../svg/XynapseLogo";
  ...
  function onGetStarted() {
    void auth.login(true).then((success) => {
      if (success) {
        ...
        ideMessenger.post("showToast", ["info", "🎉 Welcome to Xynapse!"]);
      }
    });
  }
  ...
  return (
    <div className="xs:px-0 flex w-full max-w-full flex-col items-center justify-center px-4 text-center">
      <div className="xs:flex hidden">
        <XynapseLogo height={75} />
      </div>

      <p className="mb-5 mt-0 w-full text-sm">
        Log in to Xynapse Hub to get started with AI-powered coding
      </p>
