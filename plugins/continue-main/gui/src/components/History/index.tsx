...
import {
  ...
  XynapseLabHistoryView,
} from "../claw/ClawSidecarCard";
...
export function History() {
  ...
  if (xynapseSurface === "lab") {
    return <XynapseLabHistoryView />;
  }
  if (xynapseSurface === "environment") {
    return <XynapseEnvironmentCard />;
  }
      ...
      setDialogMessage(
        <ConfirmationDialog
          ...
          onConfirm={async () => {
            ...
            await ideMessenger.request("xynapse/clearRuntimeSessions", {
              workspaceDir: window.workspacePaths?.[0],
            });
            void dispatch(refreshSessionMetadata({}));

            // start a new session
            dispatch(newSession());
            dispatch(clearLastSessionId(currentSessionId));
