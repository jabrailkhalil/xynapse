...
export function processEditorContent(editorState: JSONContent) {
  ...
  let promptBlockContent: string | undefined;
  ...
  for (const p of editorState?.content || []) {
    switch (p.type) {
      case PromptBlock.name:
        ...
        // Extract hidden content from prompt block (used by council dialog etc.)
        promptBlockContent = p.attrs?.item?.content || undefined;
        break;
      case Paragraph.name:
  ...
  return { parts, contextRequests, selectedCode, slashCommandName, promptBlockContent };
}
