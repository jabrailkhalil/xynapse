...
export async function renderSlashCommandPrompt(
  ...
  // For legacy commands, prepend hidden promptBlockContent to input
  // This allows dialogs (like Council) to pass structured data without it being visible in chat
  const legacyInput = promptBlockContent
    ? (userInput ? promptBlockContent + "\n" + userInput : promptBlockContent)
    : userInput;
