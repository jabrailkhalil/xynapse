...
function InputToolbar(props: InputToolbarProps) {
  ...
  const hasCouncil = slashCommands.some((cmd: any) => cmd.name === "council");
  const hasBvc = slashCommands.some((cmd: any) => cmd.name === "bvc");
    ...
    <>
      ...
      >
        <div className="xs:gap-1.5 flex flex-row items-center gap-1">
          ...
          {!isInEdit && hasCouncil && (
            <ToolTip place="top" content="Council">
              <HoverItem
                onClick={() => {
                  const handleCouncilSubmit = (config: CouncilConfig) => {
                    dispatch(setShowDialog(false));
                    dispatch(setDialogMessage(undefined));
                    // Put structured config in hidden prompt block content
                    // Task goes as visible text after the prompt block
                    const { task, ...configWithoutTask } = config;
                    mainEditor?.commands.insertPrompt({
                      title: "council",
                      description: "Council",
                      content: JSON.stringify(configWithoutTask),
                    });
                    if (task) {
                      mainEditor?.commands.insertContent(task);
                    }
                    // Submit after editor state updates
                    setTimeout(() => {
                      props.onEnter?.({
                        useCodebase: false,
                        noContext: false,
              ...
              >
                <UserGroupIcon className="h-3 w-3 hover:brightness-125" />
              </HoverItem>
            </ToolTip>
          )}
          {!isInEdit && hasBvc && (
            <ToolTip place="top" content="BVC">
              <HoverItem
                onClick={() => {
                  const handleBvcSubmit = (config: CouncilConfig) => {
                    dispatch(setShowDialog(false));
                    dispatch(setDialogMessage(undefined));
                    const { task, ...configWithoutTask } = config;
                    mainEditor?.commands.insertPrompt({
              ...
              >
                <BeakerIcon className="h-3 w-3 hover:brightness-125" />
              </HoverItem>
            </ToolTip>
          )}
