...
function cleanXynapseRuntimeText(text: string) {
  return text
    .replace(/\[auto-compacted:.*?\]/g, "")
    .replace(/^.*?\bXynapse thinking\.\.\.\s*/gm, "")
    .replace(/^.*?\bXynapse done\b.*$/gim, "")
    .replace(/^\s*Process exited with code 0\s*$/gim, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}

function normalizeXynapseRuntimeLog(text: string) {
  const toolPattern = XYNAPSE_RUNTIME_TOOLS.join("|");
  return cleanXynapseRuntimeText(text)
    .replace(new RegExp(`\\s+(?=(?:${toolPattern})\\b)`, "g"), "\n")
    .replace(/\s+(?=✓\s)/g, "\n")
    .replace(/\s+(?=Apply\b)/g, "\n");
}

function getRuntimeBasename(filePath: string) {
  return filePath.trim().split(/[\\/]/).filter(Boolean).pop() ?? filePath;
}

function isInternalXynapseRuntimePromptPath(filePath: string) {
  return /(?:^|[\\/])\.xynapse[\\/]runtime[\\/]prompts[\\/]/i.test(
    filePath.trim(),
  );
}

function uniqRuntimeItems(items: string[]) {
  return Array.from(new Set(items.filter(Boolean)));
}

function summarizeXynapseRuntimeActivity(text: string) {
  const normalized = normalizeXynapseRuntimeLog(text);
  const changedFiles = uniqRuntimeItems(
    Array.from(
      normalized.matchAll(/\b(?:Wrote|Edited|Created|Updated)\s+([A-Za-z]:\\[^\n(]+|[^\n(]+?)\s*(?:\(|-|$)/g),
    ).map((match) => getRuntimeBasename(match[1])),
  );
  const readFiles = uniqRuntimeItems(
    Array.from(
      normalized.matchAll(/\bRead\s+([A-Za-z]:\\[^\n(]+|[^\n(]+?)\s*(?:\(|$)/g),
    )
      .map((match) => match[1].trim())
      .filter((filePath) => !isInternalXynapseRuntimePromptPath(filePath))
      .map(getRuntimeBasename),
  );
  const searches = (normalized.match(/\b(?:file_search|text_search|glob_search|grep_search)\b/g) ?? [])
    .filter((tool, index, matches) => matches.indexOf(tool) === index).length;
  const commands = normalized
    .split("\n")
    .filter((line) => line.trim().startsWith("run_command")).length;

  const parts: string[] = [];
  if (changedFiles.length > 0) {
    parts.push(
      `changed ${changedFiles.length} file${changedFiles.length === 1 ? "" : "s"}: ${changedFiles
        .slice(0, 5)
        .map((file) => `\`${file}\``)
  ...
  if (readFiles.length > 0) {
    parts.push(
      `read ${readFiles.length} file${readFiles.length === 1 ? "" : "s"}`,
    );
  }
  if (searches > 0) {
  ...
  return parts.length > 0 ? `> Xynapse activity: ${parts.join("; ")}.` : "";
}

function findXynapseRuntimeFinalAnswer(text: string) {
  const cleaned = cleanXynapseRuntimeText(text);
  const patterns = [
    /(Теперь|Готово|Исправил|Создал|Сделал|Добавил|Обновил|Внес|Проверил|Я\s+(?:создал|исправил|добавил|обновил|внес))/i,
    /\b(I\s+(?:created|fixed|updated|added|implemented)|Done:|Implemented:|Fixed:)\b/i,
  ];
  ...
  const toolPattern = new RegExp(
    `^(?:${XYNAPSE_RUNTIME_TOOLS.join("|")})\\b|^✓(?:\\s|$)|^Apply\\b|^╭─|^╰|^│`,
  );
  return normalizeXynapseRuntimeLog(cleaned)
    .split("\n")
    .filter((line) => {
      const trimmed = line.trim();
      return (
        trimmed &&
        !toolPattern.test(trimmed) &&
        !/^(?:\/\*|\/\/|\{|\}|\)|;|\.|\[|\]|<\/?[a-z])/i.test(trimmed)
      );
    })
    .join("\n")
...
function formatXynapseRuntimeContentForChat(
  text: string,
  ...
  if (!cleaned) {
    return exitCode && exitCode !== 0
      ? `Xynapse runtime finished with code ${exitCode}.`
      : "";
  }

  const hasRuntimeNoise = new RegExp(
    `\\b(?:${XYNAPSE_RUNTIME_TOOLS.join("|")})\\b|^✓(?:\\s|$)|Xynapse thinking`,
    "m",
  ).test(cleaned);
  if (!hasRuntimeNoise) {
    return cleaned;
  ...
  const activity = summarizeXynapseRuntimeActivity(cleaned);
  const finalAnswer = findXynapseRuntimeFinalAnswer(cleaned);
  const sections = [activity, finalAnswer || "Xynapse finished."].filter(
    Boolean,
  );
...
export const sessionSlice = createSlice({
  ...
  reducers: {
    ...
    startRuntimeChatTurn: (
      ...
      const runtimeUserItem: ChatHistoryItemWithMessageId = {
        message: {
          id: uuidv4(),
          role: "user",
          content: payload.prompt,
          metadata: {
            xynapseRuntimeRunId: payload.runId,
            xynapseRuntimeSurface: payload.surface,
          } as any,
        },
      ...
      const runtimeAssistantItem: ChatHistoryItemWithMessageId = {
        message: {
          id: uuidv4(),
          role: "assistant",
          content: "",
      ...
      if (
        ...
        payload.index < state.history.length &&
        state.history[payload.index]?.message.role === "user"
      ) {
        state.history = state.history
          .slice(0, payload.index)
          .concat([runtimeUserItem, runtimeAssistantItem]);
      } else {
        state.history = state.history.concat([
          runtimeUserItem,
          runtimeAssistantItem,
        ]);
      }
      state.isStreaming = true;
      state.inlineErrorMessage = undefined;
    ...
    appendRuntimeChatChunk: (
      ...
      const item = [...state.history]
        .reverse()
        .find(
          (historyItem) =>
            historyItem.message.role === "assistant" &&
            (historyItem.message.metadata as any)?.xynapseRuntimeRunId ===
              payload.runId,
        );

      if (item?.message.role === "assistant") {
        item.message.content += payload.text;
      }
    },
    finishRuntimeChatTurn: (
      ...
      if (
        ...
        item.message.content = `Xynapse runtime finished with code ${
          payload.exitCode ?? "unknown"
        }.`;
      }
      ...
      if (
        ...
        item.message.content = formatXynapseRuntimeContentForChat(
    ...
    deleteSessionMetadata: (state, { payload }: PayloadAction<string>) => {
      ...
      if (state.lastSessionId === payload) {
        state.lastSessionId = undefined;
      }
    },
    clearLastSessionId: (state, { payload }: PayloadAction<string>) => {
      if (state.lastSessionId === payload) {
        state.lastSessionId = undefined;
      }
    },
