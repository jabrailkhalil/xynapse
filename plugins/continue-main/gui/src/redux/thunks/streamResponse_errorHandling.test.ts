...
import {
  stabilizeSymbolUpdateActionOrder,
  withoutBackgroundErrorSave,
} from "./streamResponseTestUtils";
...
describe("streamResponseThunk", () => {
  it("should throw error when no chat model is selected", async () => {
    ...
    const dispatchedActions = stabilizeSymbolUpdateActionOrder(
      withoutBackgroundErrorSave(mockStoreNoModel.getActions()),
    );
    ...
    // Saving the failed draft is allowed, but no model request may be made.
  ...
  it("should throw error for other compilation errors", async () => {
    ...
    const dispatchedActions = stabilizeSymbolUpdateActionOrder(
      withoutBackgroundErrorSave(mockStore.getActions()),
    );
    expect(dispatchedActions).toEqual([
      {
        type: "chat/streamResponse/pending",
