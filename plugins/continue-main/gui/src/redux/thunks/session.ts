...
async function getMostRecentSessionId(
  ...
  const result = await ideMessenger.request("history/list", { limit: 1 });
  if (result.status === "success") {
    return result.content.at(0)?.sessionId;
  }
  return undefined;
}
...
export const deleteSession = createAsyncThunk<void, string, ThunkApiType>(
  ...
  async (id, { getState, dispatch, extra }) => {
    ...
    const deletedSession = state.session.allSessionMetadata.find(
      (session) => session.sessionId === id,
    );
    const workspaceDir =
      deletedSession?.workspaceDirectory ||
      (id === state.session.id ? window.workspacePaths?.[0] : undefined);
    const wasCurrentSession = id === state.session.id;
    ...
    await extra.ideMessenger.request("xynapse/deleteRuntimeSession", {
      sessionId: id,
      workspaceDir,
    });
    if (wasCurrentSession) {
      dispatch(newSession());
      dispatch(clearLastSessionId(id));
    }
...
/*
 this is only used for the custom focusXynapseSessionId command at the moment
*/
...
export const loadLastSession = createAsyncThunk<void, void, ThunkApiType>(
  ...
  async (_, { extra, dispatch, getState }) => {
    ...
    if (!lastSessionId) {
      lastSessionId = await getMostRecentSessionId(extra.ideMessenger);
    }
    ...
    if (session.history.length === 0) {
      ...
      if (mostRecentSessionId && mostRecentSessionId !== lastSessionId) {
        session = await getSession(extra.ideMessenger, mostRecentSessionId);
      }
    }
