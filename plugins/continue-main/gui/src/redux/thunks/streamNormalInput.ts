...
>(
  ...
  async (
    ...
    let systemMessageWithContext = systemToolsFramework
    ...
    // Add workspace directory context so the LLM always knows where the project is
    try {
      const workspaceDirs = await extra.ideMessenger.ide.getWorkspaceDirs();
      if (workspaceDirs && workspaceDirs.length > 0) {
        const dirPaths = workspaceDirs.map((dir) =>
          decodeURIComponent(dir.replace(/^file:\/\/\//, "")).replace(/\/$/, ""),
        );
        systemMessageWithContext += `\n\n<workspace_info>\nCurrent workspace directories:\n${dirPaths.map((d) => `- ${d}`).join("\n")}\nAll relative file paths are resolved relative to these directories.\n</workspace_info>`;
      }
    } catch {
      // Ignore errors fetching workspace dirs
    }

    const systemMessage = systemMessageWithContext;
    ...
    // Shadow mode: auto-approve all non-disabled tool calls
    const isShadowMode = state3.session.mode === "background";
    const effectivePolicies = isShadowMode
      ? policies.map((p) =>
          p.policy === "allowedWithPermission"
            ? { ...p, policy: "allowedWithoutPermission" as const }
            : p,
        )
      : policies;

    const autoApprovedPolicies = effectivePolicies.filter(
      ({ policy }) => policy === "allowedWithoutPermission",
    );
    const needsApprovalPolicies = effectivePolicies.filter(
