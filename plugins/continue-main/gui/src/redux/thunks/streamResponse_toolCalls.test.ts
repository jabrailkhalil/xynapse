...
import { getRootStateWithXynapse } from "./streamResponse.test";
...
describe("streamResponseThunk - tool calls", () => {
  ...
  it("should handle tool call requiring manual approval", async () => {
    ...
    const dispatchedActions = stabilizeSymbolUpdateActionOrder(
      mockStoreWithManualApproval.getActions(),
    );
  ...
  it("should handle complete user approval and tool execution flow", async () => {
    ...
    const initialActions = stabilizeSymbolUpdateActionOrder(
      mockStoreWithApproval.getActions(),
    );
