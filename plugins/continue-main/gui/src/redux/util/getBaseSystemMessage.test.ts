...
test("getBaseSystemMessage should return the correct system message based on mode", () => {
  ...
  // Test background (shadow) mode with custom message and tools
  expect(getBaseSystemMessage("background", mockModel, [mockTool])).toBe(
    "Custom Agent System Message",
  );
...
test("getBaseSystemMessage should append no-tools warning for agent/background modes without tools", () => {
  const mockModel = {
    baseChatSystemMessage: "Custom Chat System Message",
    baseAgentSystemMessage: "Custom Agent System Message",
  } as ModelDescription;
  ...
  // Test background (shadow) mode without tools
  expect(getBaseSystemMessage("background", mockModel, [])).toBe(
    "Custom Agent System Message" + NO_TOOL_WARNING,
  );
