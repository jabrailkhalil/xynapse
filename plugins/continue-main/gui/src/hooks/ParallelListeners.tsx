...
function ParallelListeners() {
  ...
  const handleConfigUpdate = useCallback(
    async (isInitial: boolean, result: FromCoreProtocol["configUpdate"][0]) => {
      ...
      // Only mark initial load as done if we actually received a valid config
      // Otherwise, allow retries to continue
      if (!isInitial || configResult.config) {
  ...
  useEffect(() => {
    async function initialLoadConfig() {
      ...
      console.log("[Xynapse GUI] config request result.status:", result.status);
      console.log("[Xynapse GUI] config request result.content:", JSON.stringify(result.content?.result?.config?.modelsByRole?.chat?.length ?? "no chat models"));
      if (result.status === "success") {
        console.log("[Xynapse GUI] dispatching handleConfigUpdate");
        await handleConfigUpdate(true, result.content);
      } else {
        console.error("[Xynapse GUI] config request FAILED:", JSON.stringify(result));
      }
