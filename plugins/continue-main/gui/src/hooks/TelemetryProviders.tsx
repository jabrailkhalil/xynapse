...
import { isXynapseTeamMember } from "../util/isXynapseTeamMember";
...
const TelemetryProviders = ({ children }: PropsWithChildren) => {
  ...
  useEffect(() => {
    ...
    // Sentry depends on both allowAnonymousTelemetry and hasXynapseEmail
    if (false) { // Telemetry disabled
  ...
  }, [allowAnonymousTelemetry, hasXynapseEmail]);

  // Conditionally wrap with ErrorBoundary when telemetry is enabled and user is Xynapse team member
  const content =
    allowAnonymousTelemetry && hasXynapseEmail ? (
