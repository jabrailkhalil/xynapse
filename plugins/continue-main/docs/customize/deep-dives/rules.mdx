---
title: "How to Create and Manage Rules in Xynapse"
...
Xynapse supports two types of rules with different behaviors:

<Columns cols={2}>
<Card title="Local Rules" icon="folder">
- **Location**: `.xynapse/rules` folder in your workspace
...
- **Location**: Stored on Xynapse Mission Control, referenced in config.yaml
...
When using Xynapse, rules are loaded in this order:
1. **Hub assistant rules** (if using a Hub-based assistant)
2. **Referenced Hub rules** (via `uses:` in config.yaml)
3. **Local workspace rules** (from `.xynapse/rules` folder)
4. **Global rules** (from `~/.xynapse/rules` folder)
...
1. Create a folder called `.xynapse/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".xynapse/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.xynapse/rules` based on your conversation.
</Info>

### Creating Hub Rules

Rules can also be created and managed on the Xynapse Mission Control:

1. **Browse existing rules**: [Explore available rules](https://hub.xynapse.dev/hub?type=rules)
2. **Create new rules**: [Create your own](https://hub.xynapse.dev/new?type=block&blockType=rules) in the Hub
3. **Copy from local rules**: Copy/paste content from your `.xynapse/rules` files to create Hub rules
...
1. Copy the content from your `.xynapse/rules/rule-name.md` file
2. Go to [Create new rule](https://hub.xynapse.dev/new?type=block&blockType=rules)
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining Xynapse Docs
---

# Xynapse Docs Standards
...
You can create project-specific rules by adding a `.xynapse/rules` folder to the root of your project and adding new rule files.
...
```md title=".xynapse/rules/typescript.md"
---
...
**Root Cause**: This happens when you have a mix of local and Hub rules, and Xynapse can't properly determine where each rule originates.

**Workaround**:
1. **For local rules**: Navigate directly to `.xynapse/rules/` folder and edit the `.md` file
2. **For Hub rules**: Go directly to your assistant page on [Xynapse Mission Control](https://hub.xynapse.dev) and edit from there
3. Keep track of which rules are local vs Hub-based to avoid confusion

<Info>
**Known Issue**: This link accuracy problem is tracked in [Linear issue CON-3084](https://linear.app/Xynapse/issue/CON-3084) and will be fixed in a future update.
</Info>
...
**Check These**:
1. **File location**: Ensure local rules are in `.xynapse/rules/` (not `.xynapse/rule/`)
...
Xynapse includes a simple default system message for [Agent mode](../../ide-extensions/agent/quick-start) and [Chat](../../ide-extensions/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.

This can be viewed in the rules section of the toolbar (see above), or in the source code [here](https://github.com/xynapse/xynapse/blob/main/core/llm/constructMessages.ts#L4).
