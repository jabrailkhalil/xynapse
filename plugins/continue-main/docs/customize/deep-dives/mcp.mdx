---
title: "How to Set Up Model Context Protocol (MCP) in Xynapse"
...
[here](https://hub.xynapse.dev/explore/mcp).
...
```yaml title=".xynapse/mcpServers/playwright-mcp.yaml"
...
## How to Set Up Xynapse Documentation Search with MCP

You can set up an MCP server to search the Xynapse documentation directly from your config. This is particularly useful for getting help with Xynapse configuration and features.

For complete setup instructions, troubleshooting, and usage examples, see the [Xynapse MCP Reference](/reference/Xynapse-mcp).

## Using JSON MCP Format from Claude, Cursor, Cline, etc

<Info>
If you're coming from another tool that uses JSON MCP format configuration files (like Claude Desktop, Cursor, or Cline), you can copy those JSON config files directly into your `.xynapse/mcpServers/` directory (note the plural "Servers") and Xynapse will automatically pick them up.

For example, place your JSON MCP config file at `.xynapse/mcpServers/mcp.json` in your workspace.
</Info>
...
`mcpServers`](https://hub.xynapse.dev/new?type=block&blockType=mcpServers) or add a local MCP
...
<Note>
When creating a standalone block file in `.xynapse/mcpServers/`, remember to include the required metadata fields (`name`, `version`, `schema`) as shown in the Quick Start example above.
</Note>
...
as well as access hosted secrets in the Xynapse Mission Control. To leverage Hub secrets, you can use the `inputs` property in your MCP env block instead of `secrets`.
