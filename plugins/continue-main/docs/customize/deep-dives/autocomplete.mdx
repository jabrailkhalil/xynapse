---
title: "Xynapse Autocomplete Setup and Configuration Guide"
description: "Step-by-step guide to setting up and configuring autocomplete in Xynapse, including Codestral, Ollama, and IDE settings."
...
## How to Set Up Autocomplete in Xynapse with Codestral (Recommended)
...
<Tabs>
  <Tab title="Hub">
  [Mistral Codestral model block](https://hub.xynapse.dev/mistral/codestral)
  </Tab>
  <Tab title="YAML">
...
## How to Set Up Autocomplete in Xynapse with Ollama (Local Model)
...
<Tabs>
  <Tab title="Hub">
  [Ollama Qwen 2.5 Coder 1.5B model block](https://hub.xynapse.dev/ollama/qwen2.5-coder-1.5b)
  </Tab>
  <Tab title="YAML">
  ```yaml title="config.yaml"
  name: My Config
...
Then, in the Xynapse panel, select this model as the default model for autocomplete.

## Autocomplete Configuration Options in Xynapse

### Autocomplete Models Available on the Xynapse Mission Control

Explore autocomplete model configurations on [the hub](https://hub.xynapse.dev/explore/models?roles=autocomplete)

### Customize Autocomplete User Settings in the Xynapse Extension
...
5. Check the output of the logs to find any potential errors: <kbd>cmd/ctrl</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> -> "Toggle Developer Tools" -> "Console" tab in VS Code, ~/.xynapse/logs/core.log in JetBrains.
...
4. Set the trigger condition (when) to `inlineSuggestionVisible && !editorReadonly`.
   This will make multi-line completion (including Xynapse and from VS Code built-in or other plugin snippets) still work, and you will see multi-line completion. However, Tab will only fill in one line at a time. Any unnecessary code can be canceled with <kbd>Esc</kbd>.
   If you need to apply all the code, just press <kbd>Tab</kbd> multiple times.

### How to Turn Off Autocomplete in Xynapse (VS Code and JetBrains)

#### VS Code

Click the "Xynapse" button in the status panel at the bottom right of the screen. The checkmark will become a "cancel" symbol and you will no longer see completions. You can click again to turn it back on.

Alternatively, open VS Code settings, search for "Xynapse" and uncheck the box for "Enable Tab Autocomplete".

You can also use the default shortcut to disable autocomplete directly using a chord: press and hold <kbd>ctrl/cmd</kbd> + <kbd>K</kbd> (Xynapse holding <kbd>ctrl/cmd</kbd>) and press <kbd>ctrl/cmd</kbd> + <kbd>A</kbd>. This will turn off autocomplete without navigating through settings.

#### JetBrains

Open Settings -> Tools -> Xynapse and uncheck the box for "Enable Tab Autocomplete".
