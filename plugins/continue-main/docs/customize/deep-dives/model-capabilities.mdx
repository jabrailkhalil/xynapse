---
title: "How to Configure Model Capabilities in Xynapse"
...
Xynapse needs to know what features your models support to provide the best experience. This guide explains how model capabilities work and how to configure them.

## What Are Model Capabilities?

Model capabilities tell Xynapse what features a model supports:
...
## How Xynapse Detects Model Capabilities

Xynapse uses a two-tier system for determining model capabilities:

### How Automatic Detection Works (Default)

Xynapse automatically detects capabilities based on your provider and model name. For example:
...
- [toolSupport.ts](https://github.com/xynapse/xynapse/blob/main/core/llm/toolSupport.ts) - Tool capability detection logic
- [@xynapse/llm-info](https://www.npmjs.com/package/@xynapse/llm-info) - Image support detection

### How to Configure Capabilities Manually

You can add capabilities to models that Xynapse doesn't automatically detect in your `config.yaml`.

<Note>
  You cannot override autodetection - you can only add capabilities. Xynapse
...
2. **Using newer models** - Xynapse doesn't yet recognize a newly released model
...
Add tool support for a model that Xynapse doesn't recognize:
...
<Warning>
  An empty capabilities array does not disable autodetection. Xynapse will
...
3. **Keep Xynapse updated** - Newer versions improve autodetection

Remember: Setting capabilities only adds to autodetection. Xynapse will still use its built-in knowledge about your model in addition to your specified capabilities.

## Model Capability Support

This matrix shows which models support tool use and image input capabilities. Xynapse auto-detects these capabilities, but you can override them if needed.
