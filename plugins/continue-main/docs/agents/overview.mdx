...
<Card title="Agents in Xynapse" icon="robot">
  Agents are custom AI workflows that combine a prompt, rules, and tools (MCPs and more) to complete specific, repeatable tasks.
  They live in the Xynapse Mission Control and can be executed from Mission Control (web interface), TUI mode, or headless automation.
</Card>
...
<Info>
  Mission Control is in beta. Please share any feedback with us in [GitHub discussions](https://github.com/xynapse/xynapse/discussions/8051).
</Info>
...
<Tabs>
  <Tab title="🌐 Mission Control">
    **Interactive web interface**
    ...
    Trigger from the Xynapse Mission Control and review results in real-time.
    
    ```bash
    # Navigate to hub.xynapse.dev/agents
...
<CardGroup cols={2}>
  <Card title="🔒 Security Scanning" icon="shield-check" href="https://hub.xynapse.dev/xynapse/snyk-continuous-ai-agent">
    
    `xynapse/snyk-continuous-ai-agent`
    
    Finds vulnerabilities and opens PRs with fixes
    
  </Card>
  
  <Card title="🐛 GitHub Management" icon="github" href="https://hub.xynapse.dev/xynapse/github-project-manager-agent">
    
    `xynapse/github-project-manager-agent`
    
    Triages issues and manages project workflows
    
  </Card>
  
  <Card title="📊 Analytics Insights" icon="chart-bar" href="https://hub.xynapse.dev/xynapse/posthog-continuous-ai-agent">
    
    `xynapse/posthog-continuous-ai-agent`
    
    Analyzes user data and creates actionable tasks
    
  </Card>
  
  <Card title="⚡ Performance Optimization" icon="gauge-high" href="https://hub.xynapse.dev/xynapse/netlify-continuous-ai-agent">
    
    `xynapse/netlify-continuous-ai-agent`
    
    Monitors Core Web Vitals and optimizes deployments
    
  </Card>
  
  <Card title="🗄️ Database Auditing" icon="database" href="https://hub.xynapse.dev/xynapse/supabase-agent">
    
    `xynapse/supabase-agent`
    
    Audits RLS security and generates migrations
    
  </Card>
  
  <Card title="🔄 Data Pipelines" icon="arrows-rotate" href="https://hub.xynapse.dev/xynapse/dlt-agent">
...
<Steps>
  <Step title="Try a Pre-configured Agent">
    Start with a cookbook agent like `xynapse/github-project-manager-agent`:
    
    ```bash
    cn --agent xynapse/github-project-manager-agent "List open issues labeled bug"
    ```
  </Step>
...
- **Start Small**: Begin with tasks you're confident Xynapse can accomplish
...
<CardGroup cols={2}>
  ...
  <Card title="Mission Control" icon="desktop" href="https://hub.xynapse.dev/hub?type=agents">
