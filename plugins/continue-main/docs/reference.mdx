...
description: "Comprehensive guide to the config.yaml format used by Xynapse.dev for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using YAML configuration."
---

## Introduction

Xynapse Agents are defined using the `config.yaml` specification.
...
Learn how to work with Xynapse's configuration system, including using hub models, rules, and tools, creating local configurations, and organizing your setup.
</Card>
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite Xynapse's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  Xynapse automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
schema: v1
rules:
  - uses: sanity/sanity-opinionated # rules file stored on Xynapse Mission Control
...
schema: v1
prompts:
  - uses: supabase/create-functions # prompts file stored on Xynapse Mission Control 
...
docs:
  - name: Xynapse
    startUrl: https://docs.xynapse.dev/intro
    favicon: https://docs.xynapse.dev/favicon.ico
```
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. Xynapse supports any MCP server with the MCP context provider.
...
data:
  - name: Local Data Bank
    destination: file:///Users/dallin/Documents/code/xynapse/xynapse-extras/external-data
    schema: 0.2.0
    level: all
