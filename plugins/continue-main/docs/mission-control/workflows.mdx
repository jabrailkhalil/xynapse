...
<Steps>
  ...
  <Step title="Start from scratch or a template">
    ...
    In Mission Control → Workflows, create a [new workflow](https://hub.xynapse.dev/workflows/new).
<AccordionGroup>
  ...
  <Step title="Select Repository & Agent">
    ...
    <Tip>
      Need a new Agent? Create one in [Mission Control → Agents](https://hub.xynapse.dev/new?type=agent), then return to this screen.
    </Tip>
...
<CardGroup>
  <Card title="Create a Workflow Now" icon="workflow" href="https://hub.xynapse.dev/workflows">
  </Card>
