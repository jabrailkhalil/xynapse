---
title: "Xynapse CLI (cn) Overview"
description: "Command-line interface for automated coding tasks, scripting, and headless development workflows with Xynapse's AI coding capabilities"
...
**Xynapse enables developers to ship faster with Continuous AI.**
...
- A [Xynapse Mission Control](https://hub.xynapse.dev) account (recommended) or local configuration
...
That's it! You're ready to start automating with Xynapse CLI.

[Xynapse with CLI Quickstart →](/cli/quick-start)

## Two Ways to Use Xynapse CLI

Xynapse CLI offers two distinct modes designed for different workflows:
...
<Info>
  **Tool Permissions in TUI vs Headless Mode**
  ...
  Xynapse CLI tools have three permission levels:
...
## Why developers love Xynapse CLI

- **Works in your terminal**: Not another chat window. Not another IDE. Xynapse CLI meets you where you already work, with the tools you already love.
- **Takes action**: Xynapse CLI can directly edit files, run commands, and create commits. Need more? Check out our [MCPs](/customize/deep-dives/mcp).
...
- Access Xynapse Mission Control models and configurations

## Xynapse Mission Control Integration

Xynapse CLI integrates seamlessly with [Xynapse Mission Control](https://hub.xynapse.dev) for:

### API Access

Get an API key for automation workflows:

1. Visit [Xynapse Mission Control API Keys](https://hub.xynapse.dev/settings/api-keys)
...
1. Visit [Xynapse Mission Control Secrets](https://hub.xynapse.dev/settings/secrets)
...
<Card title="Installation Guide" href="/cli/install">
  Install Xynapse CLI and set up your environment
</Card>
