---
title: "Using Ollama with Xynapse: A Developer's Guide"
description: "Complete guide to setting up Ollama with Xynapse for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
## How to Configure Ollama with Xynapse

There are multiple ways to configure Ollama models in Xynapse:

### Method 1: Using Hub Model Blocks in Local config.yaml

The easiest way is to use [pre-configured model blocks](/reference#local-blocks) from the Xynapse Mission Control in your local configuration:

```yaml title="~/.xynapse/configs/config.yaml"
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures Xynapse
  to use `model: deepseek-r1:32b`, but the actual model must be installed:
  ```bash
  # Check what the block expects (view on hub.xynapse.dev)
...
Xynapse can automatically detect available Ollama models. You can configure this in your YAML:
...
3. Xynapse will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, Xynapse will dynamically populate
...
async def create_user(user: User):
    # Xynapse will help complete this implementation
...
Ollama with Xynapse provides a powerful local development environment for AI-assisted coding. You now have complete control over your AI models, ensuring privacy and enabling offline development workflows.

---

_This guide is based on Ollama v0.11.x and Xynapse v1.1.x. Please check for updates regularly._
