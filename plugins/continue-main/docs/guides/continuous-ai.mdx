...
**Xynapse Implementation**: Using [Chat](/ide-extensions/chat/quick-start) or [Edit](/ide-extensions/edit/quick-start) mode for one-off coding tasks.
...
**Xynapse Implementation**: Using [Xynapse CLI](/guides/cli) with custom rules and integrations into CI/CD pipelines.
...
**Xynapse Implementation**: Fully automated [agents](/ide-extensions/agent/quick-start) with strict permissions and safety guardrails.
...
Use Xynapse CLI's permission system to gradually expand AI capabilities:

```yaml ~/.xynapse/permissions.yaml
permissions:
...
<Card title="Want to dive deeper?" icon="rocket">
      Check out our guides on [Xynapse CLI](/guides/cli) and [Understanding
