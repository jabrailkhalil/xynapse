---
title: "Automated Security Scanning with Snyk MCP and Xynapse"
...
sidebarTitle: "Snyk Security Scanning with Xynapse"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Xynapse's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Xynapse Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Xynapse agent with Snyk MCP.
</Note>
 <Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        cn --agent xynapse/snyk-continuous-ai-agent
        ```
...
<Steps>
  ...
  <Step title="Connect Snyk MCP via Xynapse Mission Control">
    Visit the [Snyk Continuous AI Agent](https://hub.xynapse.dev/xynapse/snyk-continuous-ai-agent) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      Once installed, Snyk MCP tools become available to your Xynapse agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Xynapse agent.
    </Info>

  </Step>

  <Step title="Add Secure-at-Inception Rules">
    Install the [Snyk Secure-at-Inception rules](https://hub.xynapse.dev/snyk/secure-at-inception) from Mission Control to enable automatic security scanning.
    ...
    **How to add rules to your agent:**
    ...
    3. Rules apply globally to all your Xynapse sessions
...
Now you can use natural language prompts to run comprehensive security scans. The Xynapse agent automatically calls the appropriate Snyk MCP tools.
    ...
    steps:
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Xynapse CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Xynapse CLI to generate mitigation suggestions
            ...
            cn --agent xynapse/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Xynapse CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Xynapse CLI access** - Available as an environment variable when Xynapse generates AI mitigation suggestions
...
<Info>
  ...
  - **AI Mitigation**: Uses Xynapse CLI to generate actionable mitigation steps
...
Implement automated security policies using Xynapse's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.

<Note>
  **Coming Soon**: These security guardrail prompts will be available as pre-configured rules on the Xynapse Mission Control for easy installation.
</Note>
...
<Check>
  **Verification Steps:** - Snyk MCP is installed via [Xynapse
  Hub](https://hub.xynapse.dev/snyk/snyk-mcp) - Secure-at-Inception rules are
  [enabled](https://hub.xynapse.dev/snyk/secure-at-inception) - Authentication
