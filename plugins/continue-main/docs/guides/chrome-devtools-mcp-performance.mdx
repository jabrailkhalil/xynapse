...
description: "Measure and optimize web performance with Chrome DevTools MCP, automated performance traces, and Core Web Vitals monitoring using Xynapse."
...
This guide shows you how to leverage these features through natural language with Xynapse CLI!
...
- [Xynapse CLI](https://docs.xynapse.dev/guides/cli) (`npm i -g @xynapse/cli`)
- [Chrome DevTools MCP](https://hub.xynapse.dev) configured
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in performance analysis
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        Visit the [Chrome Dev Tools Agent](https://hub.xynapse.dev/xynapse/chrome-dev-tools-agent) on Xynapse Mission Control and click **"Install Agent"** or run:
        
        ```bash
        cn --agent xynapse/chrome-dev-tools-agent
        ```

        This agent includes:
        - **Optimized prompts** for performance analysis and debugging
        - **Built-in rules** for consistent formatting and error handling
        - **[Chrome DevTools MCP](https://hub.xynapse.dev/google/chrome-devtools-mcp)** for reliable browser automation
      </Step>
  ...
  <Tab title="🛠️ Manual Setup">
    <Steps>
      <Step title="Add Chrome DevTools MCP to Your Assistant">
        Visit the [Chrome DevTools MCP](https://hub.xynapse.dev/google/chrome-devtools-mcp) on Xynapse Mission Control and add it to your assistant, or add this to your configuration:
...
Now you can use natural language prompts to analyze web performance. The Xynapse agent automatically calls the appropriate Chrome DevTools MCP tools.
...
This workflow automatically validates web performance on pull requests using the Xynapse CLI in headless mode. It records performance traces, extracts Core Web Vitals, and posts a summary report as a PR comment.
...
<Card title="Continuous AI" icon="rocket">
  Your performance workflow now operates at **[Level 2 Continuous AI](https://blog.xynapse.dev/what-is-continuous-ai-a-developers-guide/)** - AI handles routine performance analysis and debugging with human oversight through review and approval of changes.
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Xynapse Mission Control" icon="circle-nodes" href="https://hub.xynapse.dev">
