---
title: "Developer & Team Workflows with Notion + Xynapse CLI"
description: "Use Xynapse CLI with Notion to generate docs, manage tasks, and automate project workflows – all through natural-language prompts."
sidebarTitle: "Notion with Xynapse"
---
...
- Use natural language to connect to the Notion API directly with Xynapse CLI for powerful automation
...
- [Xynapse CLI](https://docs.xynapse.dev/cli/overview) installed (`npm i -g @xynapse/cli`)
- A [Notion workspace](https://notion.so) with Editor (or higher) access
- Node.js 18+ installed locally
- [Xynapse account](https://hub.xynapse.dev) with **Hub access**

<Warning>

  **Agent usage requires credits** – create a Xynapse API key at
  [hub.xynapse.dev/settings/api-keys](https://hub.xynapse.dev/settings/api-keys)
...
<Steps>
  ...
  <Step title="Create Notion Integration & Get API Key">
    ...
    2. Click **+ New integration** → give it a name (e.g. "Xynapse Integration")
...
## Running Xynapse CLI with Notion API

<Card title="🚀 Choose Your Interface" icon="zap">

  Xynapse CLI offers two powerful modes for Notion automation:
...
<Accordion title="API Connection Notes">
  ...
  - Environment variable `NOTION_API_KEY` must be set before running Xynapse CLI
  - Xynapse automatically uses the API key to authenticate with Notion
  - No need for manual curl commands - just reference "the API key stored in this session"
  - For complex workflows, Xynapse maintains the API connection throughout
...
Working example that demonstrates the power of Xynapse with Notion API:
...
<CardGroup cols={1}>
  <Card title="Notion + GitHub" icon="github">
    ...
    <Info>
      ...
      Requires GitHub repository access. To add GitHub access, update your [integration settings](https://hub.xynapse.dev/settings/integrations).
...
<Accordion title="Common Issues and Solutions">
  ...
  **Connection Issues:**
  - Verify Xynapse CLI has internet access
