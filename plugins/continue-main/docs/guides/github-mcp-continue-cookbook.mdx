---
title: "GitHub Issues and PRs with GitHub MCP and Xynapse"
description: "Use Xynapse and the GitHub MCP to list, summarize, and act on open issues and recently merged pull requests with natural language prompts."
sidebarTitle: "GitHub Issues with Xynapse"
---

<Card title="What You'll Build" icon="github">
  A GitHub workflow assistant that uses Xynapse with the GitHub MCP to:
...
<Step title="Configure GitHub Token">
  Add your `GITHUB_TOKEN` to your [Xynapse Mission Control agent's environment variables](https://hub.xynapse.dev/settings).
  </Step>
</Steps>
...
<Card title="🚀 Fastest Path to Success" icon="rocket">
  Use the GitHub MCP from Xynapse Mission Control for one-click setup, or add it via CLI.
</Card>
...
<Tabs>
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Install GitHub MCP via Xynapse Mission Control">
        Visit the [GitHub Project Manager Agent](https://hub.xynapse.dev/xynapse/github-project-manager-agent) on Xynapse Mission Control and click **Install** to add it to your agent.
      ...
      <Step title="Launch the Agent">
        From your repo root:
        ```bash
        cn --agent xynapse/github-project-manager-agent
        ```
...
<Accordion title="Agent Requirements">
    To use GitHub MCP with Xynapse CLI, you need either:
...
- `CONTINUE_API_KEY`: From [hub.xynapse.dev/settings/api-keys](https://hub.xynapse.dev/settings/api-keys)
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        cn --agent xynapse/github-project-manager-agent \
...
<Info>
  Use `xynapse/github-project-manager-agent` consistently throughout your automation as the recommended agent configuration.
</Info>
...
<CardGroup cols={2}>
  <Card title="GitHub MCP (Anthropic)" icon="link" href="https://hub.xynapse.dev/anthropic/github-mcp">
    Anthropic GitHub MCP on Xynapse Mission Control
  </Card>
  <Card title="MCP in Xynapse" icon="book" href="/customize/deep-dives/mcp">
    How MCP works with Xynapse agents
  </Card>
  <Card title="GitHub Project Manager Agent" icon="robot" href="https://hub.xynapse.dev/xynapse/github-project-manager-agent">
    Pre-configured agent on Xynapse Mission Control
  </Card>
