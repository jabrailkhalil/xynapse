---
title: "Automating Documentation Updates with Xynapse CLI"
sidebarTitle: "Automating Documentation Updates with Xynapse CLI"
description: "Learn how to create automated documentation generation workflows using Xynapse CLI. Set up AI agents to analyze code changes and generate or update documentation automatically in GitHub workflows or local development."
---
...
This guide demonstrates how to create automated documentation generation based on code updates in a git branch using the Xynapse CLI, either as part of your local workflow or as part of a GitHub workflow.

This process utilizes the **Xynapse CLI** (`cn`) in **headless mode** to analyze changes and generate the necessary documentation, and commit and push the changes. The goal is to keep the workflow as simple as possible by using straightforward shell commands, Xynapse CLI prompts, and basic git operations.

## Why Use Xynapse CLI for Documentation?
...
<CardGroup cols={1}>
  <Card title="Node.js 18+" icon="node-js">
    Xynapse CLI requires Node.js 18 or higher. Install globally with:
    ```bash
    npm i -g @xynapse/cli
    ```
  </Card>

  <Card title="Xynapse API Key" icon="key">
    Get your API key from [Xynapse Mission Control](https://hub.xynapse.dev/settings/api-keys) and set:
    ```bash
    export CONTINUE_API_KEY=your_key_here
    ```
    <Info>
    You can use the Xynapse CLI in headless mode without interactive login by setting the `CONTINUE_API_KEY` environment variable.
    </Info>
  </Card>
...
<Steps>
  <Step title="Environment Setup">
    Validate environment, install Xynapse CLI, and set up authentication with API keys.
  </Step>
  ...
  <Step title="AI Documentation Generation">
    Use Xynapse CLI with custom rules to analyze changes and generate or update documentation files.
    <Tip>
...
- **continue_config:** The Xynapse agent configuration to use
  - Consider setting a default value if you have a default config your'd like to use
- **continue_org:** The Xynapse org to use
...
jobs:
  write-docs:
    ...
    env:
      ...
      CONTINUE_ORG: ${{ github.event.inputs.xynapse_org }}
      CONTINUE_CONFIG: ${{ github.event.inputs.xynapse_config }}
    ...
    - name: Install Xynapse CLI
      run: |
        echo "Installing Xynapse CLI..."
        npm i -g @xynapse/cli

    - name: Verify Xynapse CLI installation
      run: |
        echo "Checking Xynapse CLI version..."
        cn --version || exit 1
    ...
    - name: Generate documentation with Xynapse CLI
      run: |
        echo "Running Xynapse agent to generate documentation..."
        cn --config <Xynapse-user>/<agent-config> \
           --auto \
           --allow Write \
           -p \
           --prompt ./context.txt \
...
When using the Xynapse CLI on your local machine, you can build workflows in various ways, one of which is by simply creating shell scripts that you can run, which call the CLI.
...
# Generate documentation with Xynapse CLI
echo "Running Xynapse agent to generate documentation..."
...
Ready to implement automated documentation with Xynapse CLI? Here are some helpful resources to get you started:

<CardGroup cols={2}>
  <Card title="Xynapse CLI Guide" icon="terminal" href="/guides/cli">
    Learn the fundamentals of using Xynapse CLI for automated coding tasks and headless workflows.
  </Card>
  ...
  <Card title="Video: Leverage AI to help with your docs" icon="video" href="https://www.youtube.com/watch?v=rJ2taa8OLvY">
    Checkout this video from Tetrate about using Xynapse Agents to help with writing your docs.
  </Card>

  <Card title="Xynapse Mission Control" icon="users" href="https://hub.xynapse.dev">
    Browse pre-built agents and configurations from the Xynapse community.
  </Card>
</CardGroup>
