...
sidebarTitle: "PostHog Analytics with Xynapse CLI"
---

<Card title="What You'll Build" icon="robot">
  A fully automated workflow that uses Xynapse CLI with the PostHog MCP to fetch analytics data, analyze user experience issues with AI, and automatically create GitHub
...
- [Xynapse CLI](https://docs.xynapse.dev/guides/cli) with **active credits** (required for API usage)
...
<Steps>
  ...
  <Step title="Set up Xynapse CLI Account & API Key">
        1. Visit [Xynapse Organizations](https://hub.xynapse.dev/settings/organizations)
        2. Sign up or log in to your Xynapse account
  ...
  <Step title="Add Required Secrets to Xynapse CLI">
    Xynapse CLI will securely store your API keys as secrets that can be referenced in prompts.
  </Step>
</Steps>

<Tip>
  Xynapse CLI handles the complex API interactions - you just need to provide
  the right prompts!
</Tip>

## Step 1: Set Up Your Credentials

First, you'll need to gather your PostHog and GitHub API credentials and add them as secrets in Xynapse CLI.
...
<Tabs>
  <Tab title="PostHog API Credentials">
    ...
    3. Name it "Xynapse CLI Session Analysis"
    ...
    8. You'll also need your POSTHOG_AUTH_HEADER value, which is simply `Bearer YOUR_API_KEY`
      ...
      <Info>
  **Xynapse Secrets**: The `POSTHOG_AUTH_HEADER` secret should be stored in
  Xynapse's secure secrets storage. This keeps your API key safe and the MCP
...
<Tab title= "Configure API Access in Xynapse CLI">
 
<Tip>
See https://docs.xynapse.dev/hub/secrets/secret-types#secret-types for adding secrets
</Tip>

You only need to configure the PostHog MCP credential - it automatically handles project selection. To add environment variables to your Xynapse Mission Control account:

1.  Go to the [Xynapse Mission Control](https://hub.xynapse.dev)
...
<Tabs>
    ...
    **Perfect for:** Immediate results with optimized prompts and built-in debugging
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        Visit the [PostHog Continuous AI Agent](https://hub.xynapse.dev/xynapse/posthog-continuous-ai-agent) on Xynapse Mission Control and click **"Install Agent"** or run:
        
        ```bash
        cn --agent xynapse/posthog-continuous-ai-agent
        ```
  ...
  <Steps>
    <Step title="Add PostHog MCP to Xynapse CLI">
      First, install the [PostHog MCP](https://hub.xynapse.dev/posthog/http-mcp) to the Xynapse CLI by
    ...
    <Step title="Add PostHog + GitHub Analysis Rules">
      Visit [PostHog GitHub Continuous AI Rules](https://hub.xynapse.dev/bekah-hawrot-weigel/posthog-github-continuous-ai-rules) and click "Use Rule" to add it to your Config.
      
      You could also
      1. Pass `--rule bekah-hawrot-weigel/posthog-github-continuous-ai-rules` to `cn` OR
      2. Copy the rules to your local `.xynapse/rules` folder. See the [Rules Guide](/customize/deep-dives/rules#how-to-create-and-manage-rules-in-Xynapse).
    </Step>
    <Step title="Create Your Custom Prompts">
      Use this prompt with Xynapse CLI to analyze PostHog data and create GitHub issues:
...
<Warning>
    ...
    **Protect Your API Keys:**
    ...
    code
    - Use Xynapse CLI's secure secret storage
...
Here are practical examples of what you can build with PostHog MCP and Xynapse CLI:
...
Consider enhancing your workflow with these advanced Xynapse CLI prompts:
...
- Consider [GitHub MCP](https://hub.xynapse.dev/github/github-mcp) as an alternative (note: can be token-expensive)
- Configure [Slack MCP](https://hub.xynapse.dev/slack/slack-mcp) for alerts
- Set up [PostHog performance monitoring](https://posthog.com/docs/web-analytics)
- Join the [Xynapse Discord](https://discord.gg/Xynapse) for support
...
- [GitHub MCP on Xynapse Mission Control](https://hub.xynapse.dev/github/github-mcp) (alternative option)
- [Xynapse CLI Guide](https://docs.xynapse.dev/guides/cli)
- [Continuous AI Best Practices](https://blog.xynapse.dev/what-is-continuous-ai-a-developers-guide/)
