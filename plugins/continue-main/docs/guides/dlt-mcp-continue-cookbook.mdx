---
title: "Building Data Pipelines with dlt MCP and Xynapse"
...
sidebarTitle: "dlt Data Pipelines with Xynapse"
---

<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses Xynapse's AI agent with dlt
...
<Card title="🚀 Fastest Path to Success" icon="zap">
  Skip the manual setup and use our pre-built [dlt Agent](https://hub.xynapse.dev/xynapse/dlt-agent) that includes
...
<Steps>
  <Step title="Create a New Agent via the Xynapse Mission Control">
    Go to the [Xynapse Mission Control](https://hub.xynapse.dev) and [create a new agent](https://hub.xynapse.dev/new?type=agent).
    </Step>

  <Step title="Connect dlt MCP via Xynapse Mission Control">
    Visit the [dlt MCP on Xynapse Mission Control](https://hub.xynapse.dev/dlthub/dlt-mcp) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      2. **Manual configuration**: Add the MCP to your `~/.xynapse/config.json` under the `mcpServers` section

      Once installed, dlt MCP tools become available to your Xynapse agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    To use the pre-built [dlt Agent](https://hub.xynapse.dev/xynapse/dlt-agent), you need either:
      - **Xynapse CLI Pro Plan** with the models add-on, OR
      - **Your own API keys** added to Xynapse Mission Control secrets (same as manual setup)
...
<Card title="Understanding the Difference" icon="info-circle">
  ...
  **[dlt+ MCP](https://hub.xynapse.dev/dlthub/dlt-plus-mcp)** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **[dlt MCP](https://hub.xynapse.dev/dlthub/dlt-mcp)** is the right choice. Consider **[dlt+ MCP](https://hub.xynapse.dev/dlthub/dlt-plus-mcp)** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The Xynapse agent automatically calls the appropriate dlt MCP tools.

<Info>
You can add prompts to your agent's configuration for easy access in future sessions. Go to your agent in the [Xynapse Mission Control](https://hub.xynapse.dev), click **Edit**, and add prompts under the **Prompts** section.
</Info>

<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use Xynapse in VS Code, JetBrains, or other supported IDEs
...
## Continuous Data Pipelines with GitHub Actions
  ...
  This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](https://hub.xynapse.dev/dlthub/dlt-assistant). Consider [remixing this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to add your organization's specific validation rules.

### Add GitHub Secrets

Navigate to **Repository Settings → Secrets and variables → Actions** and add:

- `CONTINUE_API_KEY`: Your Xynapse API key from [hub.xynapse.dev/settings/api-keys](https://hub.xynapse.dev/settings/api-keys)
- Any required database credentials for your destination

<Info>
      The workflow uses the pre-built [dlt Agent](https://hub.xynapse.dev/xynapse/dlt-agent) with `--agent xynapse/dlt-agent`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations. You can [remix this agent](/guides/understanding-configs#how-to-get-started-with-hub-configs) to customize the validation rules and prompts for your specific pipeline requirements.
</Info>

### Create Workflow File

This workflow automatically validates your dlt data pipelines on pull requests using the Xynapse CLI in [headless mode](/cli/overview#headless-mode%3A-production-automation). It inspects pipeline schemas, checks for errors, and posts a summary report as a PR comment. The workflow can also be triggered manually via `workflow_dispatch`.
  ...
  validate-pipeline:
    ...
    steps:
      ...
      - name: Install Xynapse CLI
        run: |
          npm install -g @xynapse/cli
          echo "✅ Xynapse CLI installed"
      ...
      - name: Comment Pipeline Report on PR
        ...
        run: |
          REPORT=$(cn --agent xynapse/dlt-agent \
...
Implement automated pipeline quality checks using Xynapse's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
...
<Check>
  **Verification Steps:**
  - dlt MCP is installed via [Xynapse Mission Control](https://hub.xynapse.dev/dlthub/dlt-mcp)
...
<CardGroup cols={2}>
  ...
  <Card title="Xynapse Mission Control" icon="plug" href="https://hub.xynapse.dev">
    Explore more MCP integrations and agents
  </Card>
  <Card
    title="dlt Blog: MCP Deep Dive"
    icon="newspaper"
    href="https://dlthub.com/blog/deep-dive-assistants-mcp-Xynapse"
  >
    Learn about AI agents, MCP, and Xynapse integration
  </Card>
  <Card
    title="MCP Concepts"
    icon="puzzle-piece"
