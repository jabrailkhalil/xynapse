---
title: "Contributing to Xynapse with Model Context Protocol (MCP)"
description: "Use the Xynapse Docs MCP to write cookbooks, guides, and documentation with AI-powered workflows."
sidebarTitle: "Xynapse Docs MCP Cookbook"
---

<Card title="What You'll Build" icon="book-open-cover">
  Master using the Xynapse Docs MCP to contribute documentation, create cookbooks, and maintain consistency across Xynapse's docs - all through natural language prompts.
</Card>
...
- Read: [Contributing to Xynapse Documentation](/CONTRIBUTING) for setup instructions
- Forked the [xynapse/xynapse](https://github.com/xynapse/xynapse) repository
- Node.js 20+ and Xynapse CLI installed

<Warning>
  This cookbook assumes you've read the setup in the [CONTRIBUTING guide](/CONTRIBUTING). If you haven't, start there first.
</Warning>

## Xynapse Docs MCP Setup

<Card title="Fastest Path to Success" icon="rocket">
  Use the pre-built [Docs Assistant - Mintlify agent](https://hub.xynapse.dev/xynapse/docs-mintlify) that includes the Xynapse Docs MCP and is ready to use immediately.
</Card>

<Tabs>
  <Tab title="⚡ Quick Start (Recommended)">
    ```bash
    # From your Xynapse docs directory
    cn --config xynapse/docs-mintlify
    ```

    This agent includes:
    - **Xynapse Docs MCP** for searching Xynapse documentation
  ...
  <Tab title="🛠️ Custom Agent">
    ...
    1. [Xynapse Docs MCP](https://hub.xynapse.dev/xynapse/xynapse-docs-mcp)
    2. [Mintlify Technical Writing Rule](https://hub.xynapse.dev/mintlify/technical-writing-rule)
...
## What is the Xynapse Docs MCP?

<Card title="Xynapse Docs MCP" icon="circle-info">
  A Model Context Protocol server built with [Mintlify's MCP generation](https://www.mintlify.com/blog/generate-mcp-servers-for-your-docs) that enables semantic search across Xynapse documentation. [Learn more →](/reference/Xynapse-mcp)
</Card>

The MCP helps you:
- **Find examples** from existing documentation
- **Maintain consistency** with established patterns
- **Source accurate information** about Xynapse features
...
Cookbooks show how to use Xynapse CLI with specific tools or services. Here's how to create one using the Xynapse Docs MCP:
...
<Steps>
  <Step title="Research Existing Cookbooks">
    ...
    **Prompt:**
    ```
    "Show me the structure of existing MCP cookbooks in the Xynapse docs.
  ...
  <Step title="Source Official Documentation">
    **Prompt:**
    ```
    "Using the Xynapse Docs MCP, find information about how GitHub MCP works.
    ...
    The agent will:
    - Search Xynapse docs for MCP patterns
  ...
  <Step title="Add to Navigation">
    **Prompt:**
    ```
    "Add my new github-mcp-Xynapse-cookbook.mdx to the docs.json navigation
...
<Info>
  **Pro Tip:** The agent uses the Xynapse Docs MCP to maintain consistency with existing cookbooks automatically.
</Info>
...
<Steps>
  ...
  <Step title="Make Targeted Updates">
    **Prompt:**
    ...
    to include information about the new Slack MCP server. Use the Xynapse
...
<Steps>
  <Step title="Research Similar Content">
    **Prompt:**
    ```
    "I want to create a guide for setting up Xynapse with Amazon Bedrock.
    Search the Xynapse docs for similar model provider setup guides and
  ...
  <Step title="Create the Guide">
    **Prompt:**
    ...
    Use the Xynapse Docs MCP to find accurate information about Xynapse's
...
**Prompt:**
```
"Create a cookbook for using Xynapse with PostgreSQL MCP. Follow the same
...
Use the Xynapse Docs MCP to find MCP configuration patterns and search the
...
**Prompt:**
```
"Create a cookbook showing how to use Xynapse to analyze Sentry errors.
...
**Prompt:**
```
"I want to create a cookbook for using Xynapse to work with OpenAPI specs.

Show how to:
1. Load OpenAPI specs into Xynapse's context
2. Generate API client code from specs
3. Create tests based on API endpoints
4. Update documentation when APIs change

Use the Xynapse Docs MCP to find how context providers work, then combine
...
<CardGroup cols={2}>
  <Card title="Multi-Source Research" icon="magnifying-glass">
    ```bash
    "Use the Xynapse Docs MCP to understand how agents
    work in Xynapse, then search the web for Anthropic's
    Computer Use MCP. Create a cookbook showing how to
    combine Xynapse agents with Computer Use for
    automated testing."
    ```
  </Card>
  ...
  <Card title="Cross-Reference Documentation" icon="link">
    ```bash
    "Find all mentions of MCP servers across Xynapse
  ...
  <Card title="Example Extraction" icon="code">
    ...
    configuration from the Xynapse docs. Format them
...
Add automated documentation checks to your PR workflow using the Xynapse Docs MCP agent:
...
jobs:
  check-docs:
    ...
    steps:
      ...
      - name: Install Xynapse CLI
        run: npm i -g @xynapse/cli
      ...
      - name: Analyze Changes for Documentation Needs
        ...
        run: |
          ...
          # Use Xynapse agent to check if docs need updates
          ...
          cn --config xynapse/docs-mintlify -p "$PROMPT" --auto > analysis.md || {
...
- [Xynapse Docs MCP Cookbook](/guides/Xynapse-docs-mcp-cookbook)

*This analysis was generated using the Xynapse Docs MCP agent.*
EOF
...
<Info>
  This workflow uses the Xynapse Docs MCP agent to analyze code changes and automatically comment on PRs when documentation updates are recommended.
</Info>
...
<Steps>
  ...
  <Step title="Publish to Xynapse Mission Control">
    Share your docs MCP on [Xynapse Mission Control](https://hub.xynapse.dev/new?type=mcp) so others can use it with Xynapse agents.
  </Step>
</Steps>

<Info>
  The [Xynapse Docs MCP](/reference/Xynapse-mcp) itself was built this way!
</Info>
...
| Task | Prompt |
|:-----|:-------|
| **Find structure** | "Show me the structure of existing cookbooks in Xynapse docs" |
| **Create cookbook** | "Create a cookbook for [tool] MCP following the dlt cookbook structure" |
| **Update docs** | "Update [file] to include information about [feature], using Xynapse Docs MCP to find examples" |
| **Add navigation** | "Add [file] to docs.json under the [section] section" |
| **Check consistency** | "Review this file for consistency with other Xynapse documentation" |
| **Fix formatting** | "Review for Mintlify formatting issues and fix any problems" |
| **Extract examples** | "Find all examples of [topic] in Xynapse docs" |
| **Research feature** | "Use Xynapse Docs MCP to explain how [feature] works in Xynapse" |

---

## Resources

### Xynapse Documentation
- [Contributing Guide](/CONTRIBUTING) - Setup and submission process
- [Xynapse Docs MCP Reference](/reference/Xynapse-mcp) - MCP server details
...
### Community
- [Xynapse Discord](https://discord.gg/Xynapse) - #documentation channel
- [GitHub Discussions](https://github.com/xynapse/xynapse/discussions)
...
<CardGroup cols={2}>
  ...
  <Card title="Use Docs Agent" icon="robot" href="https://hub.xynapse.dev/xynapse/docs-mintlify">
  ...
  <Card title="Join Discord" icon="discord" href="https://discord.gg/Xynapse">
