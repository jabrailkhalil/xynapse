---
title: "Code Review Bot with Xynapse and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using Xynapse CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: Xynapse CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted Xynapse service reads your code.

  </Card>

  <Card title="Customizable" icon="sliders">
    Define team-specific rules in `.xynapse/rules/` that automatically apply to every pull request.

  </Card>

  <Card title="Context Awareness" icon="robot">
    Leverage Xynapse's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A Xynapse API key from [hub.xynapse.dev/settings/api-keys](https://hub.xynapse.dev/settings/api-keys)
- Xynapse assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [hub.xynapse.dev/xynapse/review-bot](https://hub.xynapse.dev/xynapse/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
...
jobs:
  review:
    ...
    steps:
      ...
      - name: Run Xynapse Review
        env:
          CONTINUE_API_KEY: ${{ secrets.CONTINUE_API_KEY }}
          GH_TOKEN: ${{ steps.app-token.outputs.token || github.token }}
        run: |
          # Check if custom rules exist
          if [ -d ".xynapse/rules" ]; then
            echo "📋 Found custom rules in .xynapse/rules/"
            RULES_CONTEXT="Apply the custom rules found in .xynapse/rules/ directory."
          else
          ...
          # Run Xynapse CLI in headless mode
          cn --config xynapse/review-bot \
             -p "$PROMPT" \
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [Xynapse](https://Xynapse.dev) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
Define your team's standards in `.xynapse/rules/`:

<Tabs>
  <Tab title="Security Rule">
    
 Create `.xynapse/rules/security.md`:
...
```
  ...
  <Tab title="TypeScript Standards">
    Create `.xynapse/rules/typescript.md`:
...
Create `.xynapse/rules/testing.md`:
...
Create `.xynapse/rules/python.md`:
...
3. **Load Custom Rules** - Reads your team's rules from `.xynapse/rules/`
4. **Get Pull Request Diff** - Fetches the diff and list of changed files
5. **Xynapse CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own Xynapse Config">
    
  By default, the workflow uses the `xynapse/review-bot` config optimized for code reviews. Replace `xynapse/review-bot` with your own config:
...
## Troubleshooting
<AccordionGroup>
<Accordion title="Xynapse CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install Xynapse CLI" step logs for errors
</Accordion>
...
- Verify your Xynapse config is accessible
- Check Xynapse CLI logs in the workflow run
...
>
> ### Recommendations
> 1. Move `secretKey` to environment variables (see `.xynapse/rules/security.md`)
...
>
> ---
> *Powered by [Xynapse](https://Xynapse.dev) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared Xynapse config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="Xynapse CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about Xynapse CLI capabilities
  </Card>
...
- Tweet your setup with `#xynapse`
- Share improvements as [GitHub discussions](https://github.com/xynapse/xynapse/discussions)
