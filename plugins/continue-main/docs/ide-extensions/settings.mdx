---
title: Settings
description: Configure Xynapse through VS Code's streamlined settings interface
---
...
<CardGroup cols={3}>
  <Card title="Settings Icon" icon="gear">
    Click the gear in the Xynapse sidebar
  </Card>
  <Card title="VS Code Settings" icon="sliders">
    File → Preferences → Settings → Extensions → Xynapse
  </Card>
...
<img
  src="/images/Xynapse-settings-card-layout.png"
  alt="Xynapse Settings Panel - Card-based Layout"
/>
## Core Settings
...
<Tabs>
  <Tab title="Interface">
    <AccordionGroup>
      ...
      <Accordion title="Behavior" icon="wand-magic-sparkles">
        ...
        | Tool Rejection | Xynapse after tool rejection |
  ...
  <Tab title="Autocomplete">
    ...
    <img
      ...
      alt="Xynapse Autocomplete Settings"
    />
  ...
  <Tab title="Indexing">
    Control how Xynapse understands your codebase:

    <img
      src="/images/indexing-settings.png"
      alt="Xynapse Indexing Settings"
    />
    ...
    <Note>
      Indexing enables Xynapse to understand your entire codebase structure, significantly improving context awareness and suggestions.
    </Note>
    ...
    | Only use system message tools | Xynapse will not attempt to use native tool calling and will only use system message tools |
    | @Codebase: use tool calling only | @codebase context provider will only use tool calling for code retrieval |
    | Stream after tool rejection | Streaming will Xynapse after the tool call is rejected |
