...
async function loadRules(ide: IDE) {
  ...
  return { rules: dedupeRulesByName(rules), errors };
}

function dedupeRulesByName(rules: RuleWithSource[]): RuleWithSource[] {
  const seenNames = new Set<string>();
  const deduped: RuleWithSource[] = [];
...
export default async function doLoadConfig(options: {
  ...
  await SentryLogger.setup(false, await ide.getUniqueId(), ideInfo);
