...
import XynapseProxyContextProvider from "../../context/providers/XynapseProxyContextProvider";
...
import XynapseProxy from "../../llm/llms/stubs/XynapseProxy";
...
import { getWorkspaceXynapseRuleDotFiles } from "../getWorkspaceXynapseRuleDotFiles";
import { loadXynapseConfigFromJson } from "../load";
...
import { loadXynapseConfigFromYaml } from "../yaml/loadYaml";

async function loadRules(ide: IDE) {
  const rules: RuleWithSource[] = [];
  const errors = [];

  // Add rules from .xynapserules files
  const { rules: yamlRules, errors: continueRulesErrors } =
    await getWorkspaceXynapseRuleDotFiles(ide);
  rules.unshift(...yamlRules);
  errors.push(...continueRulesErrors);

  // Add rules from markdown files in .xynapse/rules
...
export default async function doLoadConfig(options: {
  ...
  overrideConfigJson?: SerializedXynapseConfig;
  ...
  let newConfig: XynapseConfig | undefined;
  ...
  if (overrideConfigYaml || fs.existsSync(configYamlPath)) {
    const result = await loadXynapseConfigFromYaml({
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
    ...
    const result = await loadXynapseConfigFromJson(
      ide,
      ideSettings,
      ideInfo,
      uniqueId,
  ...
  const proxyContextProvider = newConfig.contextProviders?.find(
    (cp) => cp.description.title === "Xynapse-proxy",
  );
  if (proxyContextProvider) {
    (proxyContextProvider as XynapseProxyContextProvider).workOsAccessToken =
      workOsAccessToken;
  }
  ...
  // TODO: Remove Xynapse team member check once Sentry is ready for all users
  let userEmail: string | undefined;
  try {
    // Access the session info to get user email for Xynapse team member check
  ...
  const useOnPremProxy =
    controlPlane?.useXynapseForTeamsProxy === false && controlPlane?.proxyUrl;
...
): Promise<XynapseConfig> {
  Object.keys(config.modelsByRole).forEach((key) => {
    config.modelsByRole[key as ModelRole].forEach((model) => {
      if (model.providerName === "Xynapse-proxy") {
        (model as XynapseProxy).controlPlaneProxyInfo = info;
      }
    });
  });
