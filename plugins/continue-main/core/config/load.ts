...
async function finalToBrowserConfig(
  ...
  console.log(
    "[Xynapse] finalToBrowserConfig - modelsByRole counts:",
    JSON.stringify(
      Object.fromEntries(
        Object.entries(final.modelsByRole).map(([k, v]) => [k, v.length]),
      ),
    ),
  );
  console.log(
    "[Xynapse] finalToBrowserConfig - selectedModelByRole:",
    JSON.stringify(
      Object.fromEntries(
  ...
  return {
    ...
    docs: final.docs,
    tools: (() => {
      const serialized = final.tools.map(serializeTool);
      console.log(
        "[Xynapse] finalToBrowserConfig - tools count:",
        serialized.length,
        "| names:",
        serialized.map((t) => t.function.name).join(", "),
      );
      return serialized;
    })(),
