...
import {
  BrowserSerializedXynapseConfig,
  Config,
...
export function resolveSerializedConfig(
  ...
  const config = JSONC.parse(content) as unknown as SerializedXynapseConfig;
  ...
  return JSONC.parse(content) as unknown as SerializedXynapseConfig;
}
...
function loadSerializedConfig(
  workspaceConfigs: XynapseRcJson[],
  ideSettings: IdeSettings,
  ideType: IdeType,
  overrideConfigJson: SerializedXynapseConfig | undefined,
  ide: IDE,
): ConfigResult<SerializedXynapseConfig> {
  let config: SerializedXynapseConfig = overrideConfigJson!;
  if (!config) {
    try {
...
async function serializedToIntermediateConfig(
  initial: SerializedXynapseConfig,
  ide: IDE,
): Promise<Config> {
...
async function intermediateToFinalConfig({
  ...
  for (const cmd of config.slashCommands ?? []) {
    if ("source" in cmd) {
      xynapseConfig.slashCommands.push(cmd);
    } else {
      xynapseConfig.slashCommands.push({
        ...cmd,
  ...
  const inlineEditModel = getModelByRole(xynapseConfig, "inlineEdit")?.title;
  if (inlineEditModel) {
    const match = xynapseConfig.modelsByRole.chat.find(
      (m) => m.title === inlineEditModel,
    );
    if (match) {
      xynapseConfig.selectedModelByRole.edit = match;
      xynapseConfig.modelsByRole.edit = [match]; // The only option if inlineEdit role is set
    } else {
      errors.push({
        fatal: false,
  ...
  if (applyBlockModel) {
    ...
    if (match) {
      xynapseConfig.selectedModelByRole.apply = match;
      xynapseConfig.modelsByRole.apply = [match]; // The only option if applyCodeBlock role is set
    } else {
      errors.push({
        fatal: false,
  ...
  return { config: xynapseConfig, errors };
}
...
async function finalToBrowserConfig(
  final: XynapseConfig,
  ide: IDE,
): Promise<BrowserSerializedXynapseConfig> {
  console.log("[Xynapse] finalToBrowserConfig - modelsByRole counts:", JSON.stringify(
    Object.fromEntries(Object.entries(final.modelsByRole).map(([k, v]) => [k, v.length]))
  ));
  console.log("[Xynapse] finalToBrowserConfig - selectedModelByRole:", JSON.stringify(
    Object.fromEntries(Object.entries(final.selectedModelByRole).map(([k, v]) => [k, v?.title ?? null]))
  ));
  return {
...
async function handleEsbuildInstallation(
  ...
  const installCmd = "npm i esbuild@x.x.x --prefix ~/.xynapse";
  ...
  try {
    ...
    // Try resolving from ~/.xynapse/node_modules as a courtesy
    try {
      ...
      require(candidate);
      return true; // available via ~/.xynapse
    } catch {
      // Not available → show friendly instructions and opt out of building
      await ide.showToast(
        "error",
        [
          "config.ts has been deprecated and esbuild is no longer automatically installed by Xynapse.",
...
async function tryBuildConfigTs() {
  try {
    ...
    console.log(
      `Build error. Please check your ~/.xynapse/config.ts file: ${e}`,
    );
  }
}
...
async function loadXynapseConfigFromJson(
  ide: IDE,
  ideSettings: IdeSettings,
  ideInfo: IdeInfo,
  uniqueId: string,
...
export {
  ...
  loadXynapseConfigFromJson,
  type BrowserSerializedXynapseConfig,
};
