...
export const sharedConfigSchema = z
  .object({
    ...
    // `tabAutocompleteOptions` in `XynapseConfig`
...
// - SerializedXynapseConfig (config.json)
// - Config ("intermediate") - passed to config.ts
// - XynapseConfig
// - BrowserSerializedXynapseConfig (final converted to be passed to GUI)
...
//   Which don't exist on SerializedXynapseConfig/Config types, so must be added after the fact
export function modifyAnyConfigWithSharedConfig<
  ...
  | Config
  | SerializedXynapseConfig,
>(xynapseConfig: T, sharedConfig: SharedConfigSchema): T {
  const configCopy = { ...xynapseConfig };
