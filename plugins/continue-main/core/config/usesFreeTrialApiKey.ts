import { decodeSecretLocation, SecretType } from "@xynapse/config-yaml";
import { BrowserSerializedXynapseConfig, ModelDescription } from "..";

/**
 * Helper function to determine if the config uses an API key that relies on Xynapse credits (free trial or models add-on)
...
export function usesCreditsBasedApiKey(
  config: BrowserSerializedXynapseConfig | null,
): boolean {
  if (!config) {
    return false;
  }
