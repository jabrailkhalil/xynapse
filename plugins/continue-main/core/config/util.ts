...
import { ModelConfig, ModelRole } from "@xynapse/config-yaml";
import {
  XynapseConfig,
...
export function addModel(
  ...
  editConfigFile(
    ...
    (config) => {
      ...
      const folderId =
        (model as any).folderId ||
        model.requestOptions?.extraBodyProperties?.folderId;

      let requestOptions = model.requestOptions
        ? { ...model.requestOptions }
        : undefined;

      if (folderId && !requestOptions?.extraBodyProperties?.folderId) {
        requestOptions = {
          ...requestOptions,
          extraBodyProperties: {
      ...
      const experimentalToYamlRole: Partial<
        Record<keyof ExperimentalModelRoles, ModelRole>
      > = {
        inlineEdit: "edit",
        applyCodeBlock: "apply",
      };
      const yamlRole = role ? experimentalToYamlRole[role] : undefined;
      ...
      const desc: ModelConfig = {
        ...
        requestOptions,
        roles: yamlRole ? [yamlRole] : undefined,
      };
 ...
 *
 * See here for details: https://github.com/jabrailkhalil/Xynapse/issues/940
 */
