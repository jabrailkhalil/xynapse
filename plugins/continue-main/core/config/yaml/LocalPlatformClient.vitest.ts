import { FQSN, SecretResult, SecretType } from "@xynapse/config-yaml";
import {
  afterEach,
  beforeEach,
  describe,
...
describe("LocalPlatformClient", () => {
  ...
  describe("searches for secrets in local .env files", () => {
    let getXynapseDotEnv: Mock;
    beforeEach(async () => {
      const utilPaths = await import("../../util/paths");
      getXynapseDotEnv = vi.fn(() => envKeyValues);
    ...
    test("should be able to get secrets from ~/.xynapse/.env files", async () => {
      ...
      const resolvedFQSNs = await localPlatformClient.resolveFQSNs([testFQSN]);
      expect(getXynapseDotEnv).toHaveBeenCalled();
  ...
  describe("should be able to get secrets from workspace .env files", () => {
    test("should get secrets from <workspace>/.xynapse/.env and <workspace>/.env", async () => {
      ...
      const randomValueForXynapseDirDotEnv =
      ...
      testIde.readFile = vi.fn(async (fileUri: string) => {
        // fileUri should contain .xynapse/.env and not .env
        if (fileUri.match(/.*\.xynapse\/\.env.*/gi)?.length) {
          return (
            envKeyValuesString.split("\n")[0] + randomValueForXynapseDirDotEnv
          );
        }
        // filUri should contain .env and not .xynapse/.env
        else if (fileUri.match(/.*(?<!\.xynapse\/)\.env.*/gi)?.length) {
          return (
    ...
    test("should first get secrets from <workspace>/.xynapse/.env and then <workspace>/.env", async () => {
      ...
      // we check that workspace <workspace>.xynapse/.env does not override the <workspace>/.env secret
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
      ).toContain(randomValueForXynapseDirDotEnv);
      expect(
        (resolvedFQSNs[0] as SecretResult & { value: unknown })?.value,
  ...
  describe("should be able to get secrets from process.env", () => {
    ...
    beforeEach(async () => {
      ...
      const utilPaths = await import("../../util/paths");
      utilPaths.getXynapseDotEnv = vi.fn(() => ({}));
    ...
    test("should prioritize local ~/.xynapse/.env file over process.env", async () => {
    ...
    test("should prioritize workspace .env files over process.env", async () => {
      const workspaceXynapseEnvValue = "secret-from-workspace-continue-env";
      testIde.fileExists = vi.fn(async (fileUri: string) =>
        // Only mock existence for <workspace>/.xynapse/.env
        fileUri.includes(".xynapse/.env"),
      );
      testIde.readFile = vi.fn(async (fileUri: string) => {
        if (fileUri.includes(".xynapse/.env")) {
          return `${testFQSN.secretName}=${workspaceXynapseEnvValue}`;
        }
        return "";
      });
