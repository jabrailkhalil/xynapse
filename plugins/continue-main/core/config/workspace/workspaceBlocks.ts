...
import { IDE } from "../..";
import { getXynapseGlobalPath } from "../../util/paths";
  ...
  switch (blockType) {
    ...
    case "docs":
      configYaml.docs = [
        {
          name: "New docs",
          startUrl: "https://docs.xynapse.dev",
        },
      ];
      break;
    case "prompts":
...
export async function createNewWorkspaceBlockFile(
  ...
  const baseDirUri = joinPathsToUri(workspaceDirs[0], `.xynapse/${blockType}`);
...
export async function createNewGlobalRuleFile(
  ...
  try {
    const globalDir = localPathToUri(getXynapseGlobalPath());
