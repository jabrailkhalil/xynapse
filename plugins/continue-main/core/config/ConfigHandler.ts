...
type ConfigUpdateFunction = (payload: ConfigResult<XynapseConfig>) => void;
...
export class ConfigHandler {
      ...
      try {
        ...
        if (policyResponse?.policy?.allowOtherOrgs === false) {
          if (orgsWithPolicy.length === 0) {
            // Keep local config available even when org policy narrows hub orgs.
        ...
        errors.push({
          fatal: false,
          message: `Error loading Xynapse Hub assistants${e instanceof Error ? ":\n" + e.message : ""}`,
        });
      }
    } else {
  ...
  async getLocalProfiles(options: LoadAssistantFilesOptions) {
    /**
     * Users can define as many local agents as they want in a `.xynapse/agents` (or previous .xynapse/assistants) folder
     */
    ...
    if (options.includeWorkspace) {
      const assistantFiles = await getAllDotXynapseDefinitionFiles(
        this.ide,
        options,
        "assistants",
      );
      const agentFiles = await getAllDotXynapseDefinitionFiles(
        this.ide,
        options,
        "agents",
      );
  ...
  private notifyConfigListeners(result: ConfigResult<XynapseConfig>) {
  ...
  async getSerializedConfig(): Promise<
    ConfigResult<BrowserSerializedXynapseConfig>
  > {
