...
import {
  ...
  isXynapseAgentConfigFile,
  isXynapseConfigRelatedUri,
...
export class Core {
  ...
  constructor(
    ...
    try {
      // Ensure .xynapse directory is created
  ...
  private registerMessageHandlers(ideSettingsPromise: Promise<IdeSettings>) {
    ...
    on("files/created", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isXynapseAgentConfigFile)) {
        await this.configHandler.refreshAll("Local config file created");
      } else if (nonColocatedRuleUris.some(isXynapseConfigRelatedUri)) {
        await this.configHandler.reloadConfig(
          ".xynapse config-related file created",
        );
      }
    });
    ...
    on("files/deleted", async ({ data }) => {
      ...
      if (nonColocatedRuleUris.some(isXynapseAgentConfigFile)) {
        ...
        await this.configHandler.reloadConfig(
          ".xynapse config-related file deleted",
        );
      }
    });
    ...
    on("tools/preprocessArgs", async ({ data: { toolName, args } }) => {
      ...
      try {
        ...
        let errorReason =
          e instanceof XynapseError ? e.reason : XynapseErrorReason.Unknown;
        let errorMessage =
          e instanceof Error
            ? e.message
  ...
  private async handleFilesChanged({
    ...
    if (data?.uris?.length) {
      ...
      for (const uri of data.uris) {
        ...
        } else if (isXynapseConfigRelatedUri(uri)) {
          await this.configHandler.reloadConfig(
            "Local config-related file updated",
          );
        } else if (
          uri.endsWith(".xynapseignore") ||
  ...
  private async handleCompleteOnboarding(
    ...
    const { mode, provider, apiKey, folderId } = msg.data;
      ...
      case OnboardingModes.API_KEY:
        if (provider && apiKey) {
          editConfigYamlCallback = (config: ConfigYaml) =>
            setupProviderConfig(config, provider, apiKey, folderId);
        } else {
  ...
  private getContextItems = async (
    ...
    try {
      ...
      if (e instanceof Error) {
        ...
        //           "https://docs.xynapse.dev/customize/model-roles/embeddings",
        //         );
        //       }
        //     });
        // }
