...
import { ConfigYaml, DevEventName } from "@xynapse/config-yaml";
import * as JSONC from "comment-json";
import dotenv from "dotenv";

import { IdeType, SerializedXynapseConfig } from "../";
...
const XYNAPSE_GLOBAL_DIR = (() => {
  const configPath = process.env.XYNAPSE_GLOBAL_DIR;
  if (configPath) {
  ...
  return path.join(os.homedir(), ".xynapse");
})();
...
export function getChromiumPath(): string {
  return path.join(getXynapseUtilsPath(), ".chromium-browser-snapshots");
}

export function getXynapseUtilsPath(): string {
  const utilsPath = path.join(getXynapseGlobalPath(), ".utils");
...
export function getGlobalXynapseIgnorePath(): string {
...
export function getXynapseGlobalPath(): string {
  // This is ~/.xynapse on mac/linux
  const continuePath = XYNAPSE_GLOBAL_DIR;
...
export function getSessionsFolderPath(): string {
  const sessionsPath = path.join(getXynapseGlobalPath(), "sessions");
...
export function getIndexFolderPath(): string {
  const indexPath = path.join(getXynapseGlobalPath(), "index");
...
export function getSharedConfigFilePath(): string {
  return path.join(getXynapseGlobalPath(), "sharedConfig.json");
}
...
export function getConfigJsonPath(): string {
  const p = path.join(getXynapseGlobalPath(), "config.json");
  return p;
}

export function getConfigYamlPath(ideType?: IdeType): string {
  const p = path.join(getXynapseGlobalPath(), "config.yaml");
  if (!fs.existsSync(p) && !fs.existsSync(getConfigJsonPath())) {
    if (ideType === "jetbrains") {
      // https://github.com/jabrailkhalil/Xynapse/pull/7224
...
export function getConfigTsPath(): string {
  const p = path.join(getXynapseGlobalPath(), "config.ts");
  if (!fs.existsSync(p)) {
    fs.writeFileSync(p, DEFAULT_CONFIG_TS_CONTENTS);
  }

  const typesPath = path.join(getXynapseGlobalPath(), "types");
  ...
  const packageJsonPath = path.join(getXynapseGlobalPath(), "package.json");
  if (!fs.existsSync(packageJsonPath)) {
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "Xynapse-config",
        version: "1.0.0",
        description: "My Xynapse Configuration",
        main: "config.js",
      }),
    );
  }
...
export function getConfigJsPath(): string {
  ...
  return path.join(getXynapseGlobalPath(), "out", "config.js");
}

export function getTsConfigPath(): string {
  const tsConfigPath = path.join(getXynapseGlobalPath(), "tsconfig.json");
...
export function getXynapseRcPath(): string {
  // Disable indexing of the config folder to prevent infinite loops
  const continuercPath = path.join(getXynapseGlobalPath(), ".xynapserc.json");
...
function getDevDataPath(): string {
  const sPath = path.join(getXynapseGlobalPath(), "dev_data");
...
function editConfigJson(
  callback: (config: SerializedXynapseConfig) => SerializedXynapseConfig,
): void {
...
export function editConfigFile(
  configJsonCallback: (
    config: SerializedXynapseConfig,
  ) => SerializedXynapseConfig,
...
function getMigrationsFolderPath(): string {
  const migrationsPath = path.join(getXynapseGlobalPath(), ".migrations");
...
export function getRemoteConfigsFolderPath(): string {
  const dir = path.join(getXynapseGlobalPath(), ".configs");
...
export function getXynapseDotEnv(): { [key: string]: string } {
  const filepath = path.join(getXynapseGlobalPath(), ".env");
  if (fs.existsSync(filepath)) {
...
export function getLogsDirPath(): string {
  const logsPath = path.join(getXynapseGlobalPath(), "logs");
...
export function getGlobalFolderWithName(name: string): string {
  return path.join(getXynapseGlobalPath(), name);
}
...
export function getRepoMapFilePath(): string {
  return path.join(getXynapseUtilsPath(), "repo_map.txt");
}

export function getEsbuildBinaryPath(): string {
  return path.join(getXynapseUtilsPath(), "esbuild");
}
...
export function getLocalEnvironmentDotFilePath(): string {
  return path.join(getXynapseGlobalPath(), ".local");
}

export function getStagingEnvironmentDotFilePath(): string {
  return path.join(getXynapseGlobalPath(), ".staging");
}

export function getDiffsDirectoryPath(): string {
  const diffsPath = path.join(getXynapseGlobalPath(), ".diffs"); // .replace(/^C:/, "c:"); ??
