...
export class HistoryManager {
  private createEmptySession(sessionId: string): Session {
    return {
      history: [],
  ...
  private pruneMissingSessionFromList(sessionId: string): void {
    const sessionsListFile = getSessionsListPath();
    if (!fs.existsSync(sessionsListFile)) {
      return;
    }
    ...
    if (filteredSessions.length !== sessionsList.length) {
      fs.writeFileSync(
        sessionsListFile,
        JSON.stringify(filteredSessions, undefined, 2),
      );
    }
  }
  ...
  load(sessionId: string): Session {
    const sessionFile = getSessionFilePath(sessionId);
    if (!fs.existsSync(sessionFile)) {
      // Session metadata can outlive the underlying file after manual cleanup.
      this.pruneMissingSessionFromList(sessionId);
    ...
    try {
      ...
      console.warn(`Error loading session ${sessionId}: ${e}`);
      return this.createEmptySession(sessionId);
    }
  }
