...
export function getCleanUriPath(uri: string) {
  // URI callers normally use forward slashes, but a few public helpers also
  // receive native Windows paths. Normalize those before extracting a basename
  // so dots in parent directory names are never mistaken for file extensions.
