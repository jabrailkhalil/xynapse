...
export async function inferResolvedUriFromRelativePath(
  ...
  let dirs = dirCandidates ?? (await ide.getWorkspaceDirs());

  // Fallback: if no workspace dirs, try to use the current file's directory
  if (dirs.length === 0) {
    try {
      const currentFile = await ide.getCurrentFile();
      if (currentFile?.path) {
        // Extract directory from current file URI
        const lastSlash = currentFile.path.lastIndexOf("/");
        if (lastSlash > 0) {
          const currentDir = currentFile.path.substring(0, lastSlash);
          dirs = [currentDir];
        }
      }
    } catch {
      // Ignore errors from getCurrentFile
    }
  }

  if (dirs.length === 0) {
    // Don't throw - return a fallback path instead of crashing the GUI
    console.warn(
      "[Xynapse] No workspace dirs found, using home directory as fallback",
    );
