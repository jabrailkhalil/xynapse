 ...
 * @throws XynapseError if validation fails
 */
export function validateMultiEdit(args: unknown): {
  ...
  if (typeof args !== "object" || !args || !("edits" in args)) {
    throw new XynapseError(
      XynapseErrorReason.MultiEditEditsArrayRequired,
  ...
  if (edits.length === 0) {
    throw new XynapseError(
      XynapseErrorReason.MultiEditEditsArrayEmpty,
  ...
  for (let i = 0; i < edits.length; i++) {
    ...
    if (i > 0 && edit.old_string === "") {
      throw new XynapseError(
        XynapseErrorReason.FindAndReplaceNonFirstEmptyOldString,
