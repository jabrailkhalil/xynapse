...
import { getXynapseGlobalPath, readAllGlobalPromptFiles } from "../util/paths";
...
export async function getAllPromptFiles(
  ...
  // Also read from ~/.xynapse/prompts and ~/.xynapse/rules
  promptFiles.push(...readAllGlobalPromptFiles());

  const promptFilesFromRulesDirectory = readAllGlobalPromptFiles(
    path.join(getXynapseGlobalPath(), RULES_DIR_NAME),
  );
