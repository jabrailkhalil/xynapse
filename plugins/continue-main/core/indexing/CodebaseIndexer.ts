...
import { XynapseServerClient } from "../xynapseServer/stubs/client";
...
export class CodebaseIndexer {
  ...
  protected async getIndexesToBuild(): Promise<CodebaseIndex[]> {
    ...
    const xynapseServerClient = new XynapseServerClient(
      ideSettings.remoteConfigServerUrl,
      ideSettings.userToken,
    );
    if (!xynapseServerClient) {
      return [];
    }
  ...
  private isIndexingConfigSame(
    config1: XynapseConfig | undefined,
    config2: XynapseConfig,
  ) {
