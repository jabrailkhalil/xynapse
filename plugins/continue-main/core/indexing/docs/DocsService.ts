...
const siteIndexingConfigsAreEqual = (
  siteConfig1: SiteIndexingConfig,
  siteConfig2: SiteIndexingConfig,
  contConfig1: XynapseConfig | undefined,
  contConfig2: XynapseConfig,
) => {
  return (
...
export default class DocsService {
  ...
  async indexAndAdd(
    ...
    try {
      await provider.embed(["Xynapse-test-run"]);
    } catch (e) {
  ...
  private async syncDocs(
    oldConfig: XynapseConfig | undefined,
    newConfig: XynapseConfig,
    ...
    try {
      ...
      for (const doc of newConfigDocs) {
        ...
        if (currentIndexedDoc) {
          ...
          // TODO: Changes to the docs config made while Xynapse isn't running won't be caught
          if (
            oldConfigDoc &&
            ...
            if (forceReindex) {
              ...
              // This is a temperary fix to catch the changes to the docs config that were made when Xynapse isn't running
