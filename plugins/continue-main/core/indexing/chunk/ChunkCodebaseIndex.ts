...
import { IXynapseServerClient } from "../../xynapseServer/interface.js";
...
export class ChunkCodebaseIndex implements CodebaseIndex {
  ...
  constructor(
    ...
    private readonly xynapseServerClient: IXynapseServerClient,
  ...
  async *update(
    ...
    if (this.xynapseServerClient.connected) {
      try {
        const keys = results.compute.map(({ cacheKey }) => cacheKey);
        const resp = await this.xynapseServerClient.getFromIndexCache(
          keys,
          "chunks",
          repoName,
        );
