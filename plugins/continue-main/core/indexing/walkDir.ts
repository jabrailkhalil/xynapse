...
import { getGlobalXynapseIgArray } from "./xynapseignore";
...
export async function getIgnoreContext(
  ...
  // These are done separately so that .xynapseignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const continueIgnoreFile = dirFiles.find(
    (name) => name === ".xynapseignore",
  );
  ...
  const getXynapseIgnorePatterns = async () => {
    if (continueIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/.xynapseignore`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .xynapseignore - this is combined for speed
    .add(ignoreArrays[1]); // local .xynapseignore
