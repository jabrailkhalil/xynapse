import { parseProxyModelName } from "@xynapse/config-yaml";
...
export const PROVIDER_TOOL_SUPPORT: Record<string, (model: string) => boolean> =
  {
    ...
    anthropic: (model) => {
      ...
      return lower.includes("claude") && lower.includes("4-5");
    },
    azure: (model) => {
      if (
    ...
    vertexai: (model) => {
      ...
      return (
        lowerCaseModel.includes("gemini") || lowerCaseModel.includes("claude")
      );
    },
    xAI: (model) => {
      const lowerCaseModel = model.toLowerCase();
    ...
    // Verified against the current Yandex AI Studio OpenAI-compatible API:
    // every language model exposed in the Xynapse catalog supports tools,
    // including YandexGPT Lite and Alice AI Flash.
    yandex_gpt: () => true,
    novita: (model) => {
      const lower = model.toLowerCase();
