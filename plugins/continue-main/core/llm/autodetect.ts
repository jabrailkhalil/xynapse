...
function autodetectTemplateType(model: string): TemplateType | undefined {
  ...
  // Anthropic applies its own message formatting. Treat Claude model names as
  // explicitly template-free even when the provider is inferred from a name.
...
function autodetectPromptTemplates(
  ...
  if (templateType && USES_OS_MODELS_EDIT_PROMPT.includes(templateType)) {
    ...
    editTemplate = anthropicEditPrompt;
