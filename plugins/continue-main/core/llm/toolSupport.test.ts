...
describe("PROVIDER_TOOL_SUPPORT", () => {
  ...
  describe("bedrock", () => {
    ...
    it("should return false for unsupported Claude 3 Haiku and Opus models", () => {
  ...
  describe("yandex_gpt", () => {
    it.each([
      ...
      "gpt-oss-120b",
      "gpt-oss-20b",
    ])("supports tool calling for %s", (model) => {
      expect(PROVIDER_TOOL_SUPPORT.yandex_gpt(model)).toBe(true);
    });
  });
});
