import { createRuleMarkdown } from "@xynapse/config-yaml";
...
export const createRuleBlockImpl: ToolImpl = async (args, extras) => {
  ...
  const [localXynapseDir] = await extras.ide.getWorkspaceDirs();
  const ruleFilePath = createRuleFilePath(localXynapseDir, name);
