...
describe("runTerminalCommandImpl", () => {
  ...
  beforeEach(() => {
    ...
    mockGetWorkspaceDirs.mockReturnValue(
      Promise.resolve([pathToFileURL(tempDir).toString()]),
    );
  });
  ...
  afterAll(() => {
    ...
    try {
      ...
      // A deliberately detached PowerShell process can retain its cwd for a
      // moment after this test completes. This is an expected Windows lock,
      // not a product failure; the OS temp directory will reclaim it.
  ...
  describe("cwd handling", () => {
    describe("workspace directory handling", () => {
      it("should use file:// URI when available", async () => {
        const fileUri = pathToFileURL(tempDir).toString();
      ...
      it("should skip non-file URIs and use the first file:// URI", async () => {
        const workspaceDirs = [
          ...
          "untitled:/Untitled-1",
          pathToFileURL(tempDir).toString(),
