...
test("lsToolImpl resolves dot from workspace instead of disk root", async () => {
  vi.spyOn(walkDirModule, "walkDir").mockResolvedValue(["calculator.py"]);

  await lsToolImpl({ dirPath: "." }, mockExtras);

  expect(walkDirModule.walkDir).toHaveBeenCalledWith(
    "dir1/",
    mockExtras.ide,
