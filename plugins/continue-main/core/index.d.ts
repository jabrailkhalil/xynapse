...
export interface Session {
  ...
  /** Optional: per-session UI mode (chat/plan/edit/full plus legacy background) */
  mode?: MessageModes;
...
export type MessageModes = "chat" | "plan" | "agent" | "full" | "background";
...
export interface IdeSettings {
  ...
  userToken: string;
  xynapseTestEnvironment: "none" | "production" | "staging" | "local";
...
export interface SlashCommand extends SlashCommandDescription {
  run: (sdk: XynapseSDK) => AsyncGenerator<string | undefined>;
}

export interface SlashCommandWithSource extends SlashCommandDescription {
  run?: (sdk: XynapseSDK) => AsyncGenerator<string | undefined>; // Optional - only needed for legacy
...
// export type XynapseConfigSource = "local-yaml" | "local-json" | "hub-assistant" | "hub"
...
export interface XynapseUIConfig {
...
// config.json
export interface SerializedXynapseConfig {
  env?: string[];
...
export type XynapseRcJson = Partial<SerializedXynapseConfig> & {
  mergeBehavior: ConfigMergeType;
};

// config.ts - give users simplified interfaces
export interface Config {
  /** If set to true, Xynapse will collect anonymous usage data to improve the product. If set to false, we will collect nothing. Read here to learn more: https://docs.continue.dev/telemetry */
  allowAnonymousTelemetry?: boolean;
   ...
   * Xynapse will do the rest of the work to construct prompt templates, handle context items, prune context, etc.
   */
   ...
   * A CustomContextProvider requires you only to define a title and getContextItems function. When you type '@title <query>', Xynapse will call `getContextItems(query)`.
   */
  contextProviders?: (CustomContextProvider | ContextProviderWithParams)[];
  /** If set to true, Xynapse will not index your codebase for retrieval */
  disableIndexing?: boolean;
  /** If set to true, Xynapse will not make extra requests to the LLM to generate a summary title of each session. */
  disableSessionTitles?: boolean;
  /** An optional token to identify a user. Not used by Xynapse unless you write custom coniguration that requires such a token */
  userToken?: string;
  /** The provider used to calculate embeddings. If left empty, Xynapse will use transformers.js to calculate the embeddings with all-MiniLM-L6-v2 */
  embeddingsProvider?: EmbeddingsProviderDescription | ILLM;
  /** The model that Xynapse will use for tab autocompletions. */
...
// in the actual Xynapse source code
export interface XynapseConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
...
export interface BrowserSerializedXynapseConfig {
  allowAnonymousTelemetry?: boolean;
  // systemMessage?: string;
  completionOptions?: BaseCompletionOptions;
  requestOptions?: RequestOptions;
