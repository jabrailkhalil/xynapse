...
import { convertYamlMcpConfigToInternalMcpOptions } from "../../../config/yaml/yamlToXynapseConfig";
import {
  DEFAULT_IGNORE_DIRS,
...
/**
 * Loads MCP configs from JSON files in ~/.xynapse/mcpServers and workspace .xynapse/mcpServers
 */
export async function loadJsonMcpConfigs(
  ...
  const mcpDirs = workspaceDirs.map((dir) =>
    joinPathsToUri(dir, ".xynapse", "mcpServers"),
  );
  if (includeGlobal) {
  ...
  for (const { content, uri } of jsonFiles) {
    try {
      ...
      const codeFileParsed = claudeCodeLikeConfigFileSchema.safeParse(json);
      if (codeFileParsed.success) {
        if (codeFileParsed.data.mcpServers) {
          validJsonConfigs.push(
            ...Object.entries(codeFileParsed.data.mcpServers).map(
              ([name, mcpJson]) => ({
                name,
                mcpJson,
                uri,
        ...
        const projectServers = Object.values(codeFileParsed.data.projects).map(
          (v) => v.mcpServers,
        );
        ...
        const desktopFileParsed =
          claudeDesktopLikeConfigFileSchema.safeParse(json);
        if (desktopFileParsed.success) {
          validJsonConfigs.push(
            ...Object.entries(desktopFileParsed.data.mcpServers).map(
              ([name, mcpJson]) => ({
                name,
                mcpJson,
                uri,
