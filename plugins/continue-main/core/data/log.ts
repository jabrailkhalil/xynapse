...
import { XynapseConfig, IdeInfo, IdeSettings } from "../index.js";
...
export class DataLogger {
  ...
  async addBaseValues(
    ...
    if ("userAgent" in zodSchema.shape) {
      newBody.userAgent = ideInfo
        ? `${ideInfo.name}/${ideInfo.version} (Xynapse/${ideInfo.extensionVersion})`
        : "Unknown/Unknown (Xynapse/Unknown)";
    }
  ...
  async logToOneDestination(
    dataConfig: NonNullable<XynapseConfig["data"]>[number],
    event: DevDataLogEvent,
  ) {
    try {
      if (!dataConfig) {
      ...
      if (uriComponents.scheme === "https" || uriComponents.scheme === "http") {
        ...
        // If an API key is provided, use it, otherwise use the Xynapse access token
