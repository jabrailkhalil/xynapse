...
const MACOS_MDM_PATHS = [
  ...
  "/Library/Managed Preferences/dev.xynapse.app.plist",
  // User-specific MDM plist
  path.join(os.homedir(), "Library/Managed Preferences/dev.xynapse.app.plist"),
];
...
function readMdmKeysWindows(): MdmKeys | undefined {
  try {
    ...
    const regPath = "HKLM\\Software\\Xynapse\\MDM";
    const userRegPath = "HKCU\\Software\\Xynapse\\MDM";

    // Try to read from HKEY_LOCAL_MACHINE first
    try {
      // Use REG QUERY command to read registry values
      const licenseKeyOutput = execFileSync(
        "reg",
        ["query", regPath, "/v", "licenseKey"],
        {
          encoding: "utf8",
    ...
    try {
      const licenseKeyOutput = execFileSync(
        "reg",
        ["query", userRegPath, "/v", "licenseKey"],
        {
          encoding: "utf8",
          stdio: ["pipe", "pipe", "pipe"],
        },
...
const LINUX_MDM_PATHS = [
  ...
  "/etc/Xynapse/mdm.json",
  "/var/lib/Xynapse/mdm.json",
  // User-specific configuration
  path.join(os.homedir(), ".config/Xynapse/mdm.json"),
];
...
function writeMdmKeysLinux(licenseKey: string): boolean {
  try {
    ...
    const userMdmPath = path.join(os.homedir(), ".config/Xynapse/mdm.json");
