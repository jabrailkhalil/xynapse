...
import XynapseProxyAnalyticsProvider from "./analytics/XynapseProxyAnalyticsProvider.js";
import {
...
function createAnalyticsProvider(
  ...
  switch (config.provider) {
    ...
    case "continue-proxy":
      return new XynapseProxyAnalyticsProvider();
    default:
      return undefined;
  }
}
...
export class TeamAnalytics {
  ...
  static async setup(
    ...
    if (config.provider === "continue-proxy") {
      (
        TeamAnalytics.provider as XynapseProxyAnalyticsProvider
      ).controlPlaneClient = controlPlaneClient;
    }
  }
