...
docker build -t Xynapse-ubuntu-ssh . # build the image

rm Dockerfile # remove the created Dockerfile

container_name="Xynapse-ssh-container"

docker run -d -p 2222:22 --name $container_name Xynapse-ubuntu-ssh # run the container
