import { SerializedXynapseConfig } from "core";
...
const XYNAPSE_GLOBAL_DIR = path.join(__dirname, "..", ".xynapse");
if (fs.existsSync(XYNAPSE_GLOBAL_DIR)) {
  fs.rmSync(XYNAPSE_GLOBAL_DIR, { recursive: true, force: true });
}
fs.mkdirSync(XYNAPSE_GLOBAL_DIR);
...
describe("Test Suite", () => {
  ...
  beforeAll(async () => {
    ...
    const binaryPath = path.join(binaryDir, `xynapse-binary${exe}`);
    const expectedItems = [
      `xynapse-binary${exe}`,
      `esbuild${exe}`,
      "index.node",
    ...
    if (USE_TCP) {
      ...
      try {
        subprocess = spawn(binaryPath, {
          env: { ...process.env, XYNAPSE_GLOBAL_DIR },
        });
  ...
  it("should create .xynapse directory at the specified location with expected files", async () => {
    expect(fs.existsSync(XYNAPSE_GLOBAL_DIR)).toBe(true);
    ...
    const missingFiles = expectedFiles.filter((file) => {
      const filePath = path.join(XYNAPSE_GLOBAL_DIR, file);
  ...
  it("should add and delete a model from config.json", async () => {
    const model: SerializedXynapseConfig["models"][number] = {
      title: "Test Model",
      provider: "openai",
      model: "gpt-3.5-turbo",
