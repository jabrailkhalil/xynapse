...
describe("GUI Test", () => {
  ...
  before(async function () {
    ...
    await GUIActions.moveXynapseToSidebar(VSBrowser.instance.driver);
  ...
  describe("Onboarding", () => {
    ...
    it.skip(
      ...
      async () => {
        ...
        // TODO validate that xynapse has been added to list
  ...
  describe("Agent with tools", () => {
    ...
    it("should render tool call", async () => {
      ...
      const statusMessage = await TestUtils.waitForSuccess(
        () => GUISelectors.getToolCallStatusMessage(view), // Defined in extensions/vscode/e2e/test-xynapse/config.json's TOOL MOCK LLM that we are calling the exact search tool
        DEFAULT_TIMEOUT.SM,
      );

      expect(await statusMessage.getText()).contain(
        "Xynapse viewed the git diff",
      );
    ...
    it("should cancel tool", async () => {
      ...
      const text = await statusMessage.getText();
      expect(text).contain("Xynapse tried to view the git diff");
    }).timeout(DEFAULT_TIMEOUT.XL);
  });
