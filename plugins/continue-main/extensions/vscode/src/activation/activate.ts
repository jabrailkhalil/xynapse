import { getXynapseRcPath, getTsConfigPath } from "core/util/paths";
...
export async function activateExtension(context: vscode.ExtensionContext) {
  ...
  if (platformCheck.isUnsupported && !hasShownUnsupportedPlatformWarning) {
    ...
    void vscode.window.showInformationMessage(
      `Xynapse detected that you are using ${platformTarget}. Due to native dependencies, Xynapse may not be able to start`,
    );
  ...
  // Keep Assistant visible in the secondary sidebar by default on startup.
  // This mirrors expected Copilot-like behavior in the right panel.
  setTimeout(() => {
    void vscode.commands.executeCommand("xynapse.xynapseGUIView.focus");
  }, 300);
  ...
  const api = new VsCodeXynapseApi(vscodeExtension);
  const xynapsePublicApi = {
