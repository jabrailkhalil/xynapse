...
const statusBarItemText = (
  ...
  if (error) {
    return "$(alert) Xynapse (config error)";
  }
  ...
  switch (status) {
    case undefined:
      if (loading) {
        text = "$(loading~spin) Xynapse";
      } else {
        text = "Xynapse";
      }
      break;
    case StatusBarStatus.Disabled:
      text = "$(circle-slash) Xynapse";
      break;
    case StatusBarStatus.Enabled:
      text = "$(check) Xynapse";
      break;
    case StatusBarStatus.Paused:
      text = "$(debug-pause) Xynapse";
      break;
    default:
      text = "Xynapse";
  }
...
export function setupStatusBar(
  ...
  statusBarItem.command = "xynapse.openTabAutocompleteConfigMenu";
  ...
  vscode.workspace.onDidChangeConfiguration((event) => {
    if (event.affectsConfiguration(XYNAPSE_WORKSPACE_KEY)) {
      const enabled = getXynapseWorkspaceConfig().get<boolean>(
