      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude Xynapse's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the Xynapse's output
            !s.filepath.startsWith(
              "output:extension-output-xynapse.xynapse-assistant",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
