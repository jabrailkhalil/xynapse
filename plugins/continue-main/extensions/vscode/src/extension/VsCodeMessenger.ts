...
import { execFileWithoutShell } from "core/util/execFile";
import { validateGitHubRepoUrl } from "core/util/sanitization";
...
export class VsCodeMessenger {
  ...
  constructor(
    ...
    this.onWebview("toggleDevTools", (msg) => {
      vscode.commands.executeCommand("xynapse.viewLogs");
    });

    this.onWebview("reloadWindow", (msg) => {
      vscode.commands.executeCommand("workbench.action.reloadWindow");
    });
    this.onWebview("xynapse/importProfile", async () => {
      try {
        await vscode.commands.executeCommand("xynapse.profile.import");
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        void vscode.window.showErrorMessage(
          `Xynapse profile import failed: ${message}`,
        );
        throw error;
      }
    });
    this.onWebview("xynapse/runtimeDoctor", async (msg) => {
      await vscode.commands.executeCommand("xynapse.runtimeDoctor", msg.data);
    });
    this.onWebview("xynapse/runtimePrompt", async (msg) => {
      await vscode.commands.executeCommand("xynapse.runtimePrompt", msg.data);
    });
    this.onWebview("xynapse/runtimeStop", async (msg) => {
      await vscode.commands.executeCommand("xynapse.runtimeStop", msg.data);
    });
    this.onWebview("xynapse/openEnvironment", async (msg) => {
      return await vscode.commands.executeCommand(
        "xynapse.openEnvironment",
        msg.data,
      );
    });
    this.onWebview("xynapse/listLabHistory", async (msg) => {
      return await vscode.commands.executeCommand(
        "xynapse.listLabHistory",
        msg.data,
      );
    });
    this.onWebview("xynapse/openLabArtifact", async (msg) => {
      return await vscode.commands.executeCommand(
        "xynapse.openLabArtifact",
        msg.data,
      );
    });
    this.onWebview("xynapse/deleteRuntimeSession", async (msg) => {
      await vscode.commands.executeCommand(
        "xynapse.deleteRuntimeSession",
        msg.data,
      );
    });
    this.onWebview("xynapse/clearRuntimeSessions", async (msg) => {
      await vscode.commands.executeCommand(
      ...
      try {
        ...
        if (
          ...
          const selection = await vscode.window.showErrorMessage(
            "Background agents need GitHub access. Please connect your GitHub account to Xynapse.",
            "Connect GitHub",
            "Cancel",
          );
            ...
            try {
              await vscode.window.withProgress(
                ...
                async () => {
                  ...
                  const stashMessage = `Xynapse: Stashed before opening agent ${agentSessionId}`;
                  // Use an argument vector rather than shell escaping so this
                  // remains injection-safe on Windows, macOS, and Linux.
                  await execFileWithoutShell(
                    "git",
