...
import { XynapseCompletionProvider } from "../autocomplete/completionProvider";
import {
...
export class VsCodeExtension {
  ...
  private consoleView: XynapseConsoleWebviewViewProvider;
  private sidebar: XynapseGUIWebviewViewProvider;
  ...
  private completionProvider: XynapseCompletionProvider;

  private ARBITRARY_TYPING_DELAY = 2000;

  private async ensureXynapseDefaultTheme(): Promise<void> {
    const workbenchConfig = vscode.workspace.getConfiguration("workbench");
    const colorTheme = workbenchConfig.get<string>("colorTheme");

    if (!colorTheme || colorTheme === "__vs-dark") {
  ...
  private async saveLastWorkspaceFolder(): Promise<void> {
    const folder = this.getCurrentWorkspaceFolder();
    if (!folder?.uri.fsPath) {
      return;
    }

    // Development launches often pass the Xynapse source checkout itself.
    // Do not let that overwrite the user's last real test/project folder.
    if (
      fs.existsSync(path.join(folder.uri.fsPath, "plugins", "continue-main")) &&
      fs.existsSync(path.join(folder.uri.fsPath, "VSCode-win32-x64"))
    ) {
      return;
    }
  ...
  private async updateNextEditState(
    ...
    const { config: xynapseConfig } = await this.configHandler.loadConfig();
    const autocompleteModel = xynapseConfig?.selectedModelByRole.autocomplete;
    ...
    if (
      ...
      !modelSupportsNext &&
      !isNextEditTest() &&
      process.env.XYNAPSE_E2E_NON_NEXT_EDIT_TEST === "true"
    ) {
      vscode.window
        .showWarningMessage(
        ...
        .then((selection) => {
          if (selection === "Disable Next Edit") {
            ...
            vscode.commands.executeCommand(
              "xynapse.openTabAutocompleteConfigMenu",
            );
          }
        });
    }
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    void this.ensureXynapseDefaultTheme();
    ...
    this.sidebar = new XynapseGUIWebviewViewProvider(
      this.windowId,
    ...
    this.completionProvider = new XynapseCompletionProvider(
      this.configHandler,
      this.ide,
      this.sidebar.webviewProtocol,
      usingFullFileDiff,
    ...
    // LLM Log view
    this.consoleView = new XynapseConsoleWebviewViewProvider(
      this.windowId,
      this.extensionContext,
      this.core.llmLogger,
    );
    ...
    const globalRulesDir = path.join(getXynapseGlobalPath(), "rules");
    ...
    vscode.workspace.onDidChangeWorkspaceFolders(async (event) => {
      ...
      this.sidebar.webviewProtocol?.request(
        "updateWorkspacePaths",
        newPaths,
      );
      void this.saveLastWorkspaceFolder();
    });

    // TODO merge this and re-enable https://github.com/jabrailkhalil/Xynapse/pull/8364
    ...
    vscode.authentication.onDidChangeSessions(async (e) => {
      ...
      if (e.provider.id === env.AUTH_TYPE) {
        void vscode.commands.executeCommand(
          "setContext",
          "xynapse.isSignedInToControlPlane",
          true,
        );
    ...
    context.subscriptions.push(
      vscode.workspace.registerTextDocumentContentProvider(
        VsCodeExtension.xynapseVirtualDocumentScheme,
    ...
    void this.restoreLastWorkspaceFolderIfEmpty();
    void this.saveLastWorkspaceFolder();
  }

  static xynapseVirtualDocumentScheme = EXTENSION_NAME;
