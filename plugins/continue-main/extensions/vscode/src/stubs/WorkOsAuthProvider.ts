...
interface XynapseAuthenticationSession extends AuthenticationSession {
  refreshToken: string;
  expiresInMs: number;
  loginNeeded: boolean;
}
...
export class WorkOsAuthProvider implements AuthenticationProvider, Disposable {
  ...
  private async storeSessions(value: XynapseAuthenticationSession[]) {
  ...
  public async getSessions(
    scopes?: string[],
  ): Promise<XynapseAuthenticationSession[]> {
    ...
    try {
      ...
      const value = JSON.parse(data) as XynapseAuthenticationSession[];
      return value;
    } catch (e: any) {
  ...
  public async createSession(
    ...
    try {
      ...
      const session: XynapseAuthenticationSession = {
        id: uuidv4(),
