...
async function copyConfigSchema() {
  ...
  const serializedConfig =
    schema.$defs?.SerializedXynapseConfig ??
    schema.$defs?.SerializedContinueConfig;
  if (!serializedConfig?.properties) {
    throw new Error(
      "config_schema.json is missing a serialized Xynapse config definition",
    );
  }
  serializedConfig.properties.mergeBehavior = {
    type: "string",
  ...
  // The standalone VS Code source distribution does not contain the optional
  // IntelliJ extension. Copy schemas only when that sibling project exists.
  const intellijResources = path.join(
    "..",
    "intellij",
    "src",
    "main",
  ...
  if (fs.existsSync(intellijResources)) {
    fs.copyFileSync(
      "config_schema.json",
      path.join(intellijResources, "config_schema.json"),
    );
    fs.copyFileSync(
      "continue_rc_schema.json",
      path.join(intellijResources, "continue_rc_schema.json"),
    );
  } else {
    console.log("[info] Optional IntelliJ extension not present; skipping schema copy");
  }
}
...
async function generateAndCopyConfigYamlSchema() {
  ...
  await new Promise((resolve, reject) => {
    generateConfigYamlChild.on("message", (msg) => {
      if (msg.error) {
        reject(new Error("Failed to generate config.yaml schema"));
        return;
      }
      resolve();
    });
  ...
  await new Promise((resolve, reject) => {
    copyConfigSchemaChild.on("message", (msg) => {
      if (msg.error) {
        reject(new Error("Failed to copy configuration schemas"));
        return;
      }
      resolve();
    });
