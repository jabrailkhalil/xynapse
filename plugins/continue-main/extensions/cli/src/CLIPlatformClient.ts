...
import { DefaultApiInterface } from "@xynapse/sdk/dist/api";
...
export class CLIPlatformClient implements PlatformClient {
  ...
  private findSecretInLocalEnvFiles(fqsn: FQSN): SecretResult | undefined {
    ...
    // Then check in priority order: ~/.xynapse/.env, <workspace>/.xynapse/.env, <workspace>/.env
    const workspaceDir = process.cwd();
    const envPaths = [
      path.join(env.continueHome, ".env"),
      path.join(workspaceDir, ".xynapse", ".env"),
