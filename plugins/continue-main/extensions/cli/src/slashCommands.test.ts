import type { AssistantUnrolled } from "@xynapse/config-yaml";
import {
  beforeEach,
  describe,
  expect,
...
vi.mock("./session.js", () => ({
  getSessionFilePath: vi.fn(
    () => "/home/test/.xynapse/cli-sessions/continue-cli-pid-12345.json",
  ),
  ...
  describe("handleSlashCommands", () => {
    ...
    it("should use test session directory when in test mode", async () => {
      ...
      (getSessionFilePath as any).mockReturnValue(
        "/test-home/.xynapse/cli-sessions/continue-cli-test-123.json",
      );
      ...
      expect(result?.output).toContain("/test-home/.xynapse/cli-sessions/");
