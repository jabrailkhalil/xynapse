...
program
  .name("cn")
  .description(
    "Xynapse CLI - AI-powered development assistant. Starts an interactive session by default, use -p/--print for non-interactive output.",
  )
...
program
  .command("login")
  .description("Authenticate with Xynapse")
  .action(async () => {
    // Telemetry: record command invocation
...
program
  .command("logout")
  .description("Log out from Xynapse")
  .action(async () => {
    // Telemetry: record command invocation
...
program
  ...
  .option(
    "--id <storageId>",
    "Upload session snapshots to Xynapse-managed storage using the provided identifier",
  )
  .option(
