...
import { AssistantUnrolled, ModelConfig } from "@xynapse/config-yaml";
...
describe("Model Persistence End-to-End", () => {
  let testDir: string;
  let originalXynapseHome: string | undefined;
  ...
  beforeEach(() => {
    ...
    originalXynapseHome = process.env.XYNAPSE_GLOBAL_DIR;
    process.env.XYNAPSE_GLOBAL_DIR = testDir;
  ...
  afterEach(() => {
    ...
    if (originalXynapseHome) {
      process.env.XYNAPSE_GLOBAL_DIR = originalXynapseHome;
    } else {
      delete process.env.XYNAPSE_GLOBAL_DIR;
    }
  });
