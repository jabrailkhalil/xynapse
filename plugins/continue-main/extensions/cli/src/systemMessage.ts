...
function getDirectoryStructure(): string {
  try {
    const walker = new WalkerSync({
      ...
      includeEmpty: false,
      follow: false,
      ignoreFiles: [".gitignore", ".xynapseignore", ".customignore"],
    });
...
const baseSystemMessage = `You are an agent in the Xynapse CLI. Given the user's prompt, you should use the tools available to you to answer the user's question.
...
export async function constructSystemMessage(
  ...
  if (mode === "plan") {
    ...
    if (!process.env.XYNAPSE_CLI_DISABLE_COMMIT_SIGNATURE) {
      systemMessage += `\n<context name="commitSignature">When creating commits using any CLI or tool, include the following in the commit message:
Generated with [Xynapse](https://xynapse.dev)

Co-Authored-By: Xynapse <noreply@xynapse.dev>
</context>\n`;
    }
  }
