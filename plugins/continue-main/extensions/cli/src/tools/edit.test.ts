...
import { XynapseError, XynapseErrorReason } from "core/util/errors.js";
...
describe("editTool", () => {
  ...
  describe("preprocess", () => {
    it("should throw error if file has not been read", async () => {
      ...
      expect(error.reason).toBe(XynapseErrorReason.EditToolFileNotRead);
    });
    ...
    it("should throw error if file does not exist", async () => {
      ...
      const error = await editTool.preprocess!(args).catch((e) => e);
      expect(error).toBeInstanceOf(XynapseError);
      expect(error.reason).toBe(XynapseErrorReason.FileNotFound);
    });
    ...
    it("should throw error if old_string is not found", async () => {
      ...
      expect(error.reason).toBe(
        XynapseErrorReason.FindAndReplaceOldStringNotFound,
      );
    });
    ...
    it("should throw error if old_string appears multiple times and replace_all is false", async () => {
      ...
      expect(error).toBeInstanceOf(XynapseError);
      expect(error.reason).toBe(
        XynapseErrorReason.FindAndReplaceMultipleOccurrences,
      );
    });
    ...
    it("should throw error if old_string and new_string are the same", async () => {
      ...
      expect(error).toBeInstanceOf(XynapseError);
      expect(error.reason).toBe(
        XynapseErrorReason.FindAndReplaceIdenticalOldAndNewStrings,
      );
    });
  ...
  describe("run", () => {
    ...
    it("should throw error if file write fails", async () => {
      ...
      expect(error).toBeInstanceOf(XynapseError);
      expect(error.reason).toBe(XynapseErrorReason.FileWriteError);
    });
  });
