...
The Xynapse CLI uses esbuild to bundle the application along with local packages (`@xynapse/config-yaml` and `@xynapse/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
...
- Include local packages (`@xynapse/config-yaml`, `@xynapse/openai-adapters`) directly in the bundle
...
2. Users install via `npm install -g @xynapse/cli`
