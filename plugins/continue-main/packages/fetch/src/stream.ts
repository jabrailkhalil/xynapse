...
export async function* streamResponse(
  ...
  try {
    if (nodeMajorVersion >= 20) {
      // Undici responses already expose a Web ReadableStream. Wrapping one in
      // ReadableStream.from() can silently consume zero chunks in Node 20.
      // Only adapt legacy Node streams that do not implement pipeThrough().
      const stream =
        typeof (response.body as any).pipeThrough === "function"
          ? (response.body as ReadableStream<Uint8Array>)
          : (ReadableStream as any).from(response.body);
