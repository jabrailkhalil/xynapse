...
function getXynapseProxyModelName(
  packageIdentifier: PackageIdentifier,
  provider: string,
  model: string,
): string {
    ...
    for (let i = 0; i < config.models.length; i++) {
      ...
      if (encodedApiKeyLocation || encodedEnvSecretLocations) {
        config.models[i] = {
          ...
          provider: "Xynapse-proxy",
          model: getXynapseProxyModelName(
            packageIdentifier,
...
/** The additional properties that are added to the otherwise OpenAI-compatible body when requesting a Xynapse proxy */
...
export type XynapseProperties = z.infer<typeof continuePropertiesSchema>;
