...
const FILE_PATTERN =
  /(?:[a-z]:[\\/])?(?:[\w.@-]+[\\/])*[\w.@-]+\.(?:c|cc|cpp|cs|css|go|h|hpp|html|java|js|jsx|json|kt|md|php|py|rb|rs|sql|swift|toml|ts|tsx|vue|xml|ya?ml)/gi;
const ERROR_PATTERN =
  /\b(?:assert(?:ion)?|error|exception|fail(?:ed|ure)?|panic|traceback)\b|ошиб|исключен|падает|сбой|трассиров/iu;
const AMBIGUITY_PATTERN =
  /\b(?:ambiguous|either|maybe|one of|or|unclear|unknown)\b|неясн|возможно|либо|или один из|неизвест/iu;
const CONFLICT_PATTERN =
  /conflict(?:s|ing)? with|contradict(?:s|ion)?|log points to.+(?:but|while).+(?:issue|stack)|противореч|лог указывает.+(?:но|тогда как).+(?:issue|стек|описан)/iu;
...
export function computeBvcPreflight(task: string): BvcPreflightResult {
  ...
  const uniqueFiles = new Set(
    fileMatches.map((value) => value.toLowerCase().replace(/\\/g, "/")),
  );
  ...
  const triggerCouncil =
    uncertainty > 0.45 || (hardSignals >= 3 && nFiles >= 2);
...
export function evaluateBvcCritique(
  input: BvcCritiqueInput,
): BvcCritiqueDecision {
  const freeBudget = Math.max(
    0,
    input.totalBudget - input.usedBudget - input.reserve,
  );
  ...
  const coverageFactor =
    1 - Math.min(1, input.dCov / Math.max(input.epsilonNum, input.tauCov));
  const budgetFactor = Math.min(1, freeBudget / Math.max(1, input.activeRoles));
  const gain =
    ((input.uncertainty * voteExcess) / voteRange) *
    coverageFactor *
    budgetFactor;
  ...
  if (input.comparableAxes === 0 || input.dVote === undefined) {
    ...
    input.usedBudget + input.activeRoles + input.reserve >
    input.totalBudget
  ) {
    allowed = false;
