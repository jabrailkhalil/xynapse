...
This SDK provides programmatic access to Xynapse's Hub APIs and functionality.

## Overview

The Xynapse SDK is structured into separate language-specific packages, currently with implementations for TypeScript and Python, with more languages planned for the future:
...
1. OpenAPI-generated clients for the Xynapse Hub API
2. A wrapper layer that exposes a `Xynapse.from()` method to easily initialize and use Xynapse assistants with an OpenAI-compatible interface

## For End Users

End users should install the published packages directly:

- For TypeScript/JavaScript: `npm install @xynapse/sdk`
- For Python: `pip install xynapse`
