...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        name: "config",
        aliases: &[],
        summary: "Inspect Xynapse Lab config files or merged sections",
    ...
    SlashCommandSpec {
        name: "memory",
        aliases: &[],
        summary: "Inspect loaded Xynapse instruction memory files",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
        name: "init",
        aliases: &[],
        summary: "Create a starter XYNAPSE.md for this repo",
        argument_hint: None,
        resume_supported: true,
    },
    SlashCommandSpec {
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Xynapse runtime plugins",
        argument_hint: Some(
...
// Keep the fallible renderer signature aligned with the public slash-command
// contract so config failures can be propagated again without an API change.
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            ...
            ancestor.join(".xynapse").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(claw_config_home) =
        env::var("XYNAPSE_CONFIG_HOME").or_else(|_| env::var("CLAW_CONFIG_HOME"))
    {
        push_unique_root(
            &mut roots,
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".xynapse").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectClaw,
            ancestor.join(".xynapse").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        env::var("XYNAPSE_CONFIG_HOME").or_else(|_| env::var("CLAW_CONFIG_HOME"))
    {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClawConfigHome,
            claw_config_home.join("skills").join("omc-learned"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaw,
            home.join(".xynapse").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaw,
            home.join(".xynapse").join("skills").join("omc-learned"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserClaw,
            home.join(".xynapse").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".xynapse").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set XYNAPSE_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       xynapse agents".to_string(),
        "  Sources          .xynapse/agents, ~/.xynapse/agents, $XYNAPSE_CONFIG_HOME/agents"
            .to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "xynapse agents [list|help]",
            "sources": [".xynapse/agents", "~/.xynapse/agents", "$XYNAPSE_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       xynapse skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $XYNAPSE_CONFIG_HOME/skills or ~/.xynapse/skills".to_string(),
        "  Sources          .xynapse/skills, .omc/skills, .agents/skills, .codex/skills, ~/.xynapse/skills, ~/.xynapse/skills/omc-learned, ~/.omc/skills, ~/.codex/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "xynapse skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$XYNAPSE_CONFIG_HOME/skills or ~/.xynapse/skills",
            "sources": [
                ".xynapse/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       xynapse mcp [list|show <server>|help]".to_string(),
        "  Sources          .xynapse/settings.json, .xynapse/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "xynapse mcp [list|show <server>|help]",
            "sources": [".xynapse/settings.json", ".xynapse/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectClaw | DefinitionSource::ProjectCodex => "project_claw",
        DefinitionSource::UserClawConfigHome | DefinitionSource::UserCodexHome => {
            "user_claw_config_home"
        }
        DefinitionSource::UserClaw | DefinitionSource::UserCodex => "user_claw",
    }
}
...
mod tests {
    ...
    fn write_external_plugin(root: &Path, name: &str, version: &str) {
        fs::create_dir_all(root.join(".xynapse-plugin")).expect("manifest dir");
        fs::write(
            root.join(".xynapse-plugin").join("plugin.json"),
            format!(
    ...
    fn parses_supported_slash_commands() {
        ...
        assert_eq!(
            SlashCommand::parse("/model xynapse-opus"),
            Ok(Some(SlashCommand::Model {
                model: Some("xynapse-opus".to_string()),
            }))
        );
        assert_eq!(
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Xynapse runtime plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn ignores_unknown_or_runtime_bound_slash_commands() {
        ...
        assert!(
            handle_slash_command("/model xynapse", &session, CompactionConfig::default()).is_none()
        );
    ...
    fn lists_agents_from_project_and_user_roots() {
        ...
        let user_agents = user_home.join(".xynapse").join("agents");
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(help["usage"]["direct_cli"], "xynapse agents [list|help]");
    ...
    fn lists_skills_from_project_and_user_roots() {
        ...
        let project_commands = workspace.join(".xynapse").join("commands");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".xynapse").join("skills");
        let legacy_commands = workspace.join(".xynapse").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(
            ...
            "xynapse skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help.contains("Direct CLI       xynapse agents"));
        ...
        assert!(skills_help
            .contains("Install root     $XYNAPSE_CONFIG_HOME/skills or ~/.xynapse/skills"));
        assert!(skills_help.contains(".omc/skills"));
        assert!(skills_help.contains(".agents/skills"));
        assert!(skills_help.contains("~/.xynapse/skills/omc-learned"));
        ...
        assert!(sources
            .iter()
            .any(|value| value == "~/.xynapse/skills/omc-learned"));
    ...
    fn discovers_omc_skills_from_project_and_user_compatibility_roots() {
        ...
        let xynapse_config_dir = temp_dir("skills-omc-xynapse-config");
        ...
        let xynapse_config_skills = xynapse_config_dir.join("skills");
        let xynapse_config_commands = xynapse_config_dir.join("commands");
        let learned_skills = xynapse_config_dir.join("skills").join("omc-learned");
        let original_home = std::env::var_os("HOME");
        let original_xynapse_config_home = std::env::var_os("XYNAPSE_CONFIG_HOME");
        ...
        write_skill(
            &xynapse_config_skills,
            "statusline",
            "Xynapse config skill guidance",
        );
        write_legacy_command(
            &xynapse_config_commands,
            "doctor-check",
            "Xynapse config command guidance",
        );
        write_skill(&learned_skills, "learned", "Learned skill guidance");
        std::env::set_var("HOME", &user_home);
        std::env::set_var("XYNAPSE_CONFIG_HOME", &xynapse_config_dir);
        ...
        assert!(report.contains("statusline · Xynapse config skill guidance"));
        assert!(
            report.contains("doctor-check · Xynapse config command guidance · legacy /commands")
        );
        ...
        restore_env_var("XYNAPSE_CONFIG_HOME", original_xynapse_config_home);
        let _ = fs::remove_dir_all(workspace);
        let _ = fs::remove_dir_all(user_home);
        let _ = fs::remove_dir_all(xynapse_config_dir);
    }
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       xynapse mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".xynapse")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".xynapse").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".xynapse").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".xynapse/settings.json");
