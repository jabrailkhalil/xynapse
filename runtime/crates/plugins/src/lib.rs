...
const MANIFEST_RELATIVE_PATH: &str = ".xynapse-plugin/plugin.json";

#[cfg(windows)]
pub(crate) fn windows_posix_shell() -> PathBuf {
    if let Some(configured) = std::env::var_os("XYNAPSE_BASH") {
...
fn plugin_tool_command(command: &str, args: &[String]) -> Command {
    ...
    {
        let mut process = Command::new(windows_posix_shell());
...
fn load_manifest_from_path(
    ...
    let compatibility_errors = detect_xynapse_code_manifest_contract_gaps(&raw_json);
...
fn detect_xynapse_code_manifest_contract_gaps(
    raw_manifest: &Value,
    ...
    for (field, detail) in [
        (
            "skills",
            "plugin manifest field `skills` uses an unsupported plugin contract; Xynapse discovers skills from local roots such as `.xynapse/skills`, `.omc/skills`, `.agents/skills`, and `~/.omc/skills`.",
        ),
        (
            "mcpServers",
            "plugin manifest field `mcpServers` uses an unsupported plugin contract; Xynapse does not import MCP servers from plugin manifests.",
        ),
        (
            "agents",
            "plugin manifest field `agents` uses an unsupported plugin contract; Xynapse does not load plugin-managed agent markdown catalogs from plugin manifests.",
        ),
    ] {
    ...
    {
        errors.push(PluginManifestValidationError::UnsupportedManifestContract {
            detail: "plugin manifest field `commands` uses unsupported directory globs; Xynapse slash dispatch is still built-in and does not load plugin slash command markdown files.".to_string(),
        });
    }
    ...
    if let Some(hooks) = root.get("hooks").and_then(Value::as_object) {
        for hook_name in hooks.keys() {
            if !matches!(
                ...
                errors.push(PluginManifestValidationError::UnsupportedManifestContract {
                    detail: format!(
                        "plugin hook `{hook_name}` uses an unsupported lifecycle contract; Xynapse plugins currently support only PreToolUse, PostToolUse, and PostToolUseFailure."
                    ),
                });
            }
        }
...
fn run_lifecycle_commands(
    ...
    for command in commands {
        let mut process = if Path::new(command).exists() {
            #[cfg(windows)]
            let process = {
                let mut process = if Path::new(command)
                    .extension()
                    .is_some_and(|extension| extension.eq_ignore_ascii_case("sh"))
                {
                    Command::new(windows_posix_shell())
                } else {
                    Command::new("cmd")
                };
                if Path::new(command)
...
mod tests {
  ...
  "commands": [
    ...
    #[test]
    fn load_plugin_from_directory_rejects_xynapse_code_manifest_contracts_with_guidance() {
        let root = temp_dir("manifest-xynapse-code-contract");
        write_file(
            root.join(MANIFEST_FILE_NAME).as_path(),
            r#"{
  "name": "oh-my-xynapsecode",
  "version": "4.10.2",
  "description": "Xynapse Code plugin manifest",
  "hooks": {
  ...
  "mcpServers": "./.mcp.json"
        ...
        let error = load_plugin_from_directory(&root)
            .expect_err("Xynapse Code plugin manifest should fail with guidance");
        let rendered = error.to_string();
        assert!(rendered.contains("field `skills` uses an unsupported plugin contract"));
        assert!(rendered.contains("field `mcpServers` uses an unsupported plugin contract"));
        assert!(rendered.contains("field `agents` uses an unsupported plugin contract"));
        assert!(rendered.contains("field `commands` uses unsupported directory globs"));
        assert!(rendered.contains("hook `SessionStart` uses an unsupported lifecycle contract"));
