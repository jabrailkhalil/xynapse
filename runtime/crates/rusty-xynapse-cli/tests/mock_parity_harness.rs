...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("XYNAPSE_CONFIG_HOME", &workspace.config_home)
        .env("HOME", &workspace.home)
        .env("NO_COLOR", "1")
        .args([
            "--model",
            "anthropic/xynapse-sonnet-4-6",
            "--permission-mode",
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    let manifest_dir = plugin_root.join(".xynapse-plugin");
    fs::create_dir_all(&tool_dir).expect("plugin tools dir");
    fs::create_dir_all(&manifest_dir).expect("plugin manifest dir");

    let script_path = tool_dir.join("echo-json.py");
    fs::write(
        &script_path,
        "import json, os, sys\nprint(json.dumps({\"plugin\": os.environ[\"CLAWD_PLUGIN_ID\"], \"tool\": os.environ[\"CLAWD_TOOL_NAME\"], \"input\": json.load(sys.stdin)}))\n",
    )
...
fn assert_read_file_roundtrip(workspace: &HarnessWorkspace, run: &ScenarioRun) {
    ...
    let output = run.response["tool_results"][0]["output"]
        .as_str()
        .expect("tool output");
    let output_json: Value = serde_json::from_str(output).expect("read_file output should be json");
    let actual_path = PathBuf::from(
        output_json["file"]["filePath"]
            .as_str()
            .expect("read_file output path"),
    );
    assert_eq!(
        actual_path.canonicalize().expect("actual fixture path"),
        workspace
            .root
            .join("fixture.txt")
            .canonicalize()
