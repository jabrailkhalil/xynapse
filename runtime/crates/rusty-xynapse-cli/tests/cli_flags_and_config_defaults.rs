...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_xynapse"))
    ...
    assert!(stdout.contains("Model            openai/gpt-4.1-mini"));
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_xynapse"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("claw should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_xynapse"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("claw should launch");
...
fn omc_namespaced_slash_commands_surface_a_targeted_compatibility_hint() {
    ...
    assert!(stderr.contains("unknown slash command outside the REPL: /oh-my-xynapsecode:hud"));
    assert!(stderr.contains("OMC plugin command"));
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".xynapse");
    fs::create_dir_all(temp_dir.join(".xynapse")).expect("project config dir should exist");
    ...
    fs::write(
        temp_dir.join(".xynapse").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("XYNAPSE_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            xynapse doctor"));
    ...
    assert!(status_stdout.contains("Usage            xynapse status"));
...
fn command_in(cwd: &Path) -> Command {
    let mut command = Command::new(env!("CARGO_BIN_EXE_xynapse"));
