...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  xynapse --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".xynapse")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".xynapse").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
...
fn resumed_status_surfaces_persisted_model() {
    ...
    let mut session = workspace_session(&temp_dir);
    session.model = Some("xynapse-sonnet-4-6".to_string());
    session
    ...
    assert_eq!(parsed["kind"], "status");
    assert_eq!(
        parsed["model"], "xynapse-sonnet-4-6",
