...
const OFFICIAL_REPO_URL: &str = "https://github.com/jabrailkhalil/xynapse";
const OFFICIAL_REPO_SLUG: &str = "jabrailkhalil/xynapse";
const DEPRECATED_INSTALL_COMMAND: &str = "legacy standalone runtime installer";
...
Open Xynapse Lab help from the IDE for usage."
                );
            }
        }
        std::process::exit(1);
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match parse_args(&args)? {
        ...
        CliAction::Prompt {
            ...
            cli.record_prompt_history(&prompt);
    ...
    match rest[0].as_str() {
        ...
        other => {
            ...
            if joined.trim().is_empty() {
                return Err(
                    "empty prompt: provide a subcommand (run Xynapse runtime help) or a non-empty prompt string"
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn format_unknown_option(option: &str) -> String {
    ...
    message.push_str("\nRun Xynapse runtime help for usage.");
    message
}
...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    // full gpt:// URIs, so validate by the first slash instead of requiring
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    let openai_api_key_present = env::var("OPENAI_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let xai_api_key_present = env::var("XAI_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let dashscope_api_key_present = env::var("DASHSCOPE_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let yandex_api_key_present = env::var("YANDEX_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let supported_auth_present = api_key_present
        ...
        || yandex_api_key_present;
    let env_details = format!(
        "Environment       anthropic_api_key={} auth_token={} openai_api_key={} xai_api_key={} dashscope_api_key={} yandex_api_key={}",
        ...
        if openai_api_key_present { "present" } else { "absent" },
        if xai_api_key_present { "present" } else { "absent" },
        if dashscope_api_key_present { "present" } else { "absent" },
        if yandex_api_key_present { "present" } else { "absent" },
    );
...
fn check_install_source_health() -> DiagnosticCheck {
    DiagnosticCheck::new(
        ...
        DiagnosticLevel::Ok,
        format!(
            "official source of truth is {OFFICIAL_REPO_SLUG}; use the bundled Xynapse runtime"
        ),
    )
    .with_details(vec![
    ...
    .with_data(Map::from_iter([
        ...
        (
            ...
            json!("use bundled Xynapse runtime"),
        ),
        (
            "recommended_install".to_string(),
            json!("use the bundled runtime shipped with Xynapse IDE"),
        ),
    ]))
}
...
const DUMP_MANIFESTS_OVERRIDE_HINT: &str =
    "Hint: set XYNAPSE_LAB_UPSTREAM=/path/to/upstream or pass the manifest directory explicitly.";
...
fn print_bootstrap_plan(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    let phases = runtime::BootstrapPlan::xynapse_default()
        .phases()
        .iter()
...
impl LiveCli {
    fn new(
        ...
        let (session, session_state) = if let Some(session_id) = xynapse_runtime_session_id() {
            load_or_create_named_session(&session_id)?
        } else {
...
fn xynapse_runtime_session_id() -> Option<String> {
    env::var("XYNAPSE_RUNTIME_SESSION_ID")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

fn load_or_create_named_session(
    session_id: &str,
) -> Result<(SessionHandle, Session), Box<dyn std::error::Error>> {
    let handle = create_managed_session_handle(session_id)?;
    if handle.path.exists() {
        let (_resolved, session) = load_session_reference(session_id)?;
        let path = handle.path.clone();
        return Ok((handle, session.with_persistence_path(path)));
    }

    let mut session = new_cli_session()?;
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No Xynapse instruction files discovered in the current directory ancestry."
                .to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
fn init_xynapse_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  xynapse-lab --model openai/gpt-4.1-mini \"summarize this repo\""
    )?;
    writeln!(
        out,
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        let error = ApiError::ContextWindowExceeded {
            model: "xynapse-sonnet-4-6".to_string(),
        ...
        assert!(
            rendered.contains("Model            xynapse-sonnet-4-6"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("rusty-xynapse-cli-{nanos}-{unique}"))
    }
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        match original_permission_mode {
            ...
            None => std::env::remove_var("RUSTY_XYNAPSE_PERMISSION_MODE"),
        }
    ...
    fn removed_login_and_logout_subcommands_error_helpfully() {
        ...
        match parse_args(&[
            "--model=anthropic/xynapse-opus-4-6".to_string(),
            "status".to_string(),
        ])
        .expect("--model=... status should parse")
        {
            CliAction::Status {
                ...
                assert_eq!(model, "anthropic/xynapse-opus-4-6");
                assert_eq!(
                    model_flag_raw.as_deref(),
                    Some("anthropic/xynapse-opus-4-6"),
...
"#,
    ...
    #[test]
    fn format_connected_line_renders_anthropic_provider_for_xynapse_model() {
        let model = "xynapse-sonnet-4-6";

        let line = format_connected_line(model);

        assert_eq!(line, "Connected: xynapse-sonnet-4-6 via anthropic");
    }
    ...
    fn resolve_repl_model_returns_user_supplied_model_unchanged_when_explicit() {
        let user_model = "xynapse-sonnet-4-6".to_string();
    ...
    fn model_report_uses_sectioned_layout() {
        let report = format_model_report("xynapse-sonnet", 12, 4);
        assert!(report.contains("Model"));
        assert!(report.contains("Current model    xynapse-sonnet"));
    ...
    fn model_switch_report_preserves_context_summary() {
        let report = format_model_switch_report("xynapse-sonnet", "xynapse-opus", 9);
        assert!(report.contains("Model updated"));
        assert!(report.contains("Previous         xynapse-sonnet"));
        assert!(report.contains("Current          xynapse-opus"));
    ...
    fn status_line_reports_model_and_token_totals() {
        ...
        assert!(status.contains("Model            xynapse-sonnet"));
...
UU conflicted.rs",
    ...
    fn render_diff_report_shows_clean_tree_for_committed_repo() {
        ...
        git(&["config", "user.name", "Rusty Xynapse Tests"], &root);
    ...
    fn init_template_mentions_detected_rust_workspace() {
        ...
        let rendered = crate::init::render_init_xynapse_md(&workspace_root);
        assert!(rendered.contains("# XYNAPSE.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/rusty-xynapse-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p rusty-xynapse-cli"}"#)
                .contains("cargo test -p rusty-xynapse-cli")
        );
        assert_eq!(
    ...
    fn response_to_events_renders_collapsed_thinking_summary() {
        ...
        assert!(
            matches!(&events[0], AssistantEvent::Thinking { thinking, signature } if thinking == "step 1" && signature.as_deref() == Some("sig_123"))
        );
        let rendered = String::from_utf8(out).expect("utf8");
    ...
    fn stub_commands_absent_from_repl_completions() {
        let candidates =
            slash_command_completion_candidates_with_sessions("xynapse-3-5-sonnet", None, vec![]);
