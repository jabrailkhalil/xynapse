...
const STARTER_XYNAPSE_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Xynapse Lab local artifacts";
const GITIGNORE_ENTRIES: [&str; 3] = [
    ".xynapse/settings.local.json",
    ".xynapse/sessions/",
    ".xynapse/runtime/",
];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let xynapse_dir = cwd.join(".xynapse");
    artifacts.push(InitArtifact {
        name: ".xynapse/",
        status: ensure_dir(&xynapse_dir)?,
    });

    let settings_json = xynapse_dir.join("settings.json");
    artifacts.push(InitArtifact {
        name: ".xynapse/settings.json",
        status: write_file_if_missing(&settings_json, STARTER_XYNAPSE_JSON)?,
    });
    ...
    let xynapse_md = cwd.join("XYNAPSE.md");
    let content = render_init_xynapse_md(cwd);
    artifacts.push(InitArtifact {
        name: "XYNAPSE.md",
        status: write_file_if_missing(&xynapse_md, &content)?,
    });
...
pub(crate) fn render_init_xynapse_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# XYNAPSE.md".to_string(),
        String::new(),
        "This file provides guidance to Xynapse Lab when working with code in this repository."
            .to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.xynapse/settings.json`; reserve `.xynapse/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `XYNAPSE.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_xynapse_md, InitStatus};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("rusty-xynapse-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".xynapse/"));
        assert!(rendered.contains(".xynapse/settings.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("XYNAPSE.md"));
        assert!(root.join(".xynapse").is_dir());
        assert!(root.join(".xynapse").join("settings.json").is_file());
        assert!(root.join("XYNAPSE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".xynapse").join("settings.json"))
                .expect("read xynapse settings"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".xynapse/settings.local.json"));
        assert!(gitignore.contains(".xynapse/sessions/"));
        assert!(gitignore.contains(".xynapse/runtime/"));
        let xynapse_md = fs::read_to_string(root.join("XYNAPSE.md")).expect("read xynapse md");
        assert!(xynapse_md.contains("Languages: Rust."));
        assert!(xynapse_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("XYNAPSE.md"), "custom guidance\n").expect("write existing xynapse md");
        fs::write(root.join(".gitignore"), ".xynapse/settings.local.json\n")
            .expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first.render().contains("XYNAPSE.md"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".xynapse/"));
        assert!(second_rendered.contains(".xynapse/settings.json"));
        assert!(second_rendered.contains("skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("XYNAPSE.md"));
        assert!(second_rendered.contains("skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("XYNAPSE.md")).expect("read existing xynapse md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".xynapse/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".xynapse/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".xynapse/runtime/").count(), 1);
    ...
    fn artifacts_with_status_partitions_fresh_and_idempotent_runs() {
        ...
        assert_eq!(
            ...
            vec![
                ".xynapse/".to_string(),
                ".xynapse/settings.json".to_string(),
                ".gitignore".to_string(),
                "XYNAPSE.md".to_string(),
            ],
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_xynapse_md(Path::new(&root));
