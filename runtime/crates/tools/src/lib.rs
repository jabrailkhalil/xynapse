...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set Xynapse runtime settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn execute_skill(input: SkillInput) -> Result<SkillOutput, String> {
    ...
    Ok(SkillOutput {
        skill: input.skill,
        path: skill_path.display().to_string().replace('\\', "/"),
        args: input.args,
        description,
        prompt,
    })
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(xynapse_config_home) = std::env::var("XYNAPSE_CONFIG_HOME")
        .or_else(|_| std::env::var("XYNAPSE_CONFIG_DIR"))
        .or_else(|_| std::env::var("CLAW_CONFIG_HOME"))
    {
        let xynapse_config_home = std::path::PathBuf::from(xynapse_config_home);
        push_prefixed_skill_lookup_roots(&mut roots, &xynapse_config_home);
        push_skill_lookup_root(
            &mut roots,
            xynapse_config_home.join("skills").join("omc-learned"),
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".xynapse"));
    }
}
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".xynapse"));
    push_skill_lookup_root(
        roots,
...
const DEFAULT_AGENT_MODEL: &str = "xynapse-opus-4-6";
    ...
    while let Some(event) = stream.next_event().await? {
        match event {
            ...
            ApiStreamEvent::ContentBlockStart(start) => {
                ...
                {
                    pending_thinking.insert(start.index, (thinking.clone(), signature.clone()));
                }
                push_output_block(
                    start.content_block,
                    start.index,
...
fn command_exists(command: &str) -> bool {
    #[cfg(windows)]
    {
        ...
        std::env::split_paths(&path).any(|directory| {
            ...
            candidate.is_file()
                || (std::path::Path::new(command).extension().is_none()
                    && extensions
                        .iter()
...
mod tests {
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .xynapse/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let xynapse_dir = worktree.join(".xynapse");
        fs::create_dir_all(&xynapse_dir).expect("create .xynapse dir");
        ...
        fs::write(xynapse_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .xynapse/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".xynapse").join("worker-state.json");
    ...
    #[test]
    fn reasoning_survives_subagent_history_conversion() {
        use api::InputContentBlock;
        use runtime::{ContentBlock, ConversationMessage};
        let original = ConversationMessage::assistant(vec![ContentBlock::Thinking {
            thinking: "Synthetic subagent reasoning".into(),
            signature: Some("fixture-signature".into()),
        }]);
        let converted = super::convert_messages(&[original]);
        assert!(
            matches!(&converted[0].content[0], InputContentBlock::Thinking { thinking, signature }
            if thinking == "Synthetic subagent reasoning" && signature.as_deref() == Some("fixture-signature"))
        );
        let mut events = Vec::new();
        super::push_output_block(
            OutputContentBlock::Thinking {
                thinking: "Synthetic subagent reasoning".into(),
                signature: None,
            },
        ...
        assert!(
            matches!(&events[0], AssistantEvent::Thinking { thinking, .. } if thinking == "Synthetic subagent reasoning")
        );
    }
    ...
    #[test]
    fn skill_loads_project_local_xynapse_skill_prompt() {
        let _guard = env_guard();
        let root = temp_path("project-skills");
        let home = root.join("home");
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let skill_dir = workspace.join(".xynapse").join("skills").join("trace");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".xynapse/skills/trace/SKILL.md"));
    ...
    #[test]
    fn skill_loads_learned_skill_from_xynapse_config_dir() {
        let _guard = env_guard();
        let root = temp_path("xynapse-config-learned-skill");
        let home = root.join("home");
        let xynapse_config_dir = root.join("xynapse-config");
        let learned_skill_dir = xynapse_config_dir
            .join("skills")
            .join("omc-learned")
            .join("learned");
        ...
        let original_config_home = std::env::var("CLAW_CONFIG_HOME").ok();
        let original_codex_home = std::env::var("CODEX_HOME").ok();
        let original_xynapse_config_dir = std::env::var("XYNAPSE_CONFIG_DIR").ok();
        std::env::set_var("HOME", &home);
        std::env::remove_var("CLAW_CONFIG_HOME");
        std::env::remove_var("CODEX_HOME");
        std::env::set_var("XYNAPSE_CONFIG_DIR", &xynapse_config_dir);
        ...
        match original_xynapse_config_dir {
            Some(value) => std::env::set_var("XYNAPSE_CONFIG_DIR", value),
            None => std::env::remove_var("XYNAPSE_CONFIG_DIR"),
        }
        fs::remove_dir_all(root).expect("temp tree should clean up");
    }

    #[test]
    fn skill_loads_direct_skill_and_legacy_command_from_xynapse_config_dir() {
        let _guard = env_guard();
        let root = temp_path("xynapse-config-direct-skill");
        let home = root.join("home");
        let xynapse_config_dir = root.join("xynapse-config");
        let skill_dir = xynapse_config_dir.join("skills").join("statusline");
        let command_dir = xynapse_config_dir.join("commands");
        fs::create_dir_all(&skill_dir).expect("direct skill dir should exist");
        fs::create_dir_all(&command_dir).expect("command dir should exist");
        fs::write(
            skill_dir.join("SKILL.md"),
            "---\nname: statusline\ndescription: Xynapse config skill\n---\n# statusline\n",
        )
        .expect("direct skill file should exist");
        fs::write(
            command_dir.join("doctor-check.md"),
            "---\nname: doctor-check\ndescription: Xynapse config command\n---\n# doctor-check\n",
        )
        ...
        assert_eq!(direct_skill_output["description"], "Xynapse config skill");
    ...
    fn skill_loads_project_local_legacy_command_markdown() {
        ...
        let workspace = root.join("workspace");
        let nested = workspace.join("nested");
        let command_dir = workspace.join(".xynapse").join("commands");
        ...
        assert!(output["path"]
            .as_str()
            .expect("path")
            .ends_with(".xynapse/commands/team.md"));
    ...
    fn agent_fake_runner_can_persist_completion_and_failure() {
        ...
        let completed = execute_agent_with_spawn(
            AgentInput {
                ...
                model: Some("xynapse-sonnet-4-6".to_string()),
            },
            |job| {
...
"#,
    ...
    #[test]
    #[cfg(windows)]
    fn powershell_runs_on_windows() {
        let result = execute_tool(
            "PowerShell",
            &json!({"command": "Write-Output hello", "timeout": 5000}),
        )
        .expect("PowerShell should succeed");
    ...
    fn provider_runtime_client_chain_uses_only_primary_when_no_fallbacks_configured() {
        ...
        assert_eq!(client.chain[0].model, "xynapse-sonnet-4-6");
    ...
    fn provider_runtime_client_chain_primary_override_replaces_constructor_model() {
        ...
        let fallback_config = ProviderFallbackConfig::new(
            ...
            vec!["xynapse-sonnet-4-6".to_string()],
        );

        // when
        let client = ProviderRuntimeClient::new_with_fallback_config(
            "xynapse-haiku-4-5-20251213".to_string(),
            BTreeSet::new(),
            &fallback_config,
        )
        ...
        assert_eq!(client.chain[1].model, "xynapse-sonnet-4-6");
    ...
    fn provider_runtime_client_chain_skips_fallbacks_missing_credentials() {
        ...
        assert_eq!(client.chain.len(), 2);
        assert_eq!(client.chain[0].model, "xynapse-sonnet-4-6");
        assert_eq!(client.chain[1].model, "xynapse-haiku-4-5-20251213");
