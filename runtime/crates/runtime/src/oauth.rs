...
fn credentials_home_dir() -> io::Result<PathBuf> {
    if let Some(path) = std::env::var_os("XYNAPSE_CONFIG_HOME") {
        ...
        .ok_or_else(|| {
            io::Error::new(
                ...
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use XYNAPSE_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".xynapse"))
}
