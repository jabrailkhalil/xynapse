...
fn shell_command(command: &str) -> CommandWithStdin {
    #[cfg(windows)]
    let command_builder = {
        let mut command_builder = Command::new(crate::bash::default_shell_program());
...
mod tests {
    ...
    #[test]
    fn object_hook_matcher_runs_only_for_matching_tool() {
        use crate::config::RuntimeHookCommand;
        let config = RuntimeHookConfig::from_hook_commands(
            vec![RuntimeHookCommand::with_matcher(
                "exit 2",
                Some("read_file|glob_*".into()),
            )],
            vec![],
            vec![],
        );
        let runner = HookRunner::new(config);
        assert!(!runner.run_pre_tool_use("write_file", "{}").is_denied());
        assert!(runner.run_pre_tool_use("read_file", "{}").is_denied());
        assert!(runner.run_pre_tool_use("glob_search", "{}").is_denied());
    }
}
