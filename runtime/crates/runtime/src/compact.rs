...
fn summarize_block(block: &ContentBlock) -> String {
    let raw = match block {
        ContentBlock::Thinking { .. } => "[reasoning omitted]".to_string(),
    ...
    message
        ...
        .map(|block| match block {
            ContentBlock::Thinking {
                thinking,
                signature,
            } => (thinking.len() + signature.as_ref().map_or(0, String::len)) / 4 + 1,
...
mod tests {
    ...
    fn extracts_key_files_from_message_content() {
        let files = collect_key_files(&[ConversationMessage::user_text(
            "Update rust/crates/runtime/src/compact.rs and rust/crates/rusty-xynapse-cli/src/main.rs next.",
        )]);
        assert!(files.contains(&"rust/crates/runtime/src/compact.rs".to_string()));
        assert!(files.contains(&"rust/crates/rusty-xynapse-cli/src/main.rs".to_string()));
    }
    ...
    #[test]
    fn reasoning_counts_toward_budget_but_stays_out_of_summary() {
        let message = ConversationMessage::assistant(vec![ContentBlock::Thinking {
            thinking: "private reasoning fixture/file.rs".repeat(100),
            signature: Some("signature".repeat(20)),
        }]);
        assert!(super::estimate_message_tokens(&message) > 800);
        assert!(!super::summarize_block(&message.blocks[0]).contains("private reasoning"));
        assert!(super::collect_key_files(&[message.clone()]).is_empty());
        assert_eq!(super::first_text_block(&message), None);
    }
}
