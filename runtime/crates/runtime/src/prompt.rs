...
pub const FRONTIER_MODEL_NAME: &str = "Xynapse frontier model";
...
impl ProjectContext {
    ...
    pub fn discover_with_git(
        ...
        context.git_diff = if env_flag_enabled("CLAW_SKIP_GIT_DIFF") {
            None
        } else {
            read_git_diff(&context.cwd)
        };
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("XYNAPSE.md"),
            dir.join("XYNAPSE.local.md"),
            dir.join("AGENTS.md"),
            dir.join(".xynapse").join("XYNAPSE.md"),
            dir.join(".xynapse").join("instructions.md"),
        ] {
...
fn render_project_context(project_context: &ProjectContext) -> String {
    ...
    if !project_context.instruction_files.is_empty() {
        bullets.push(format!(
            "Xynapse instruction files discovered: {}.",
...
fn render_instruction_files(files: &[ContextFile]) -> String {
    let mut sections = vec!["# Xynapse instructions".to_string()];
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Xynapse runtime settings files loaded.".to_string(),
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".xynapse")).expect("nested xynapse dir");
        fs::write(root.join("XYNAPSE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("XYNAPSE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".xynapse")).expect("apps xynapse dir");
        fs::write(root.join("apps").join("XYNAPSE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".xynapse").join("instructions.md"),
            "apps dot xynapse instructions",
        )
        .expect("write apps dot xynapse instructions");
        fs::write(nested.join(".xynapse").join("XYNAPSE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".xynapse").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("XYNAPSE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("XYNAPSE.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.claw/XYNAPSE.md")),
            "XYNAPSE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("XYNAPSE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? XYNAPSE.md"));
    ...
    #[test]
    fn load_system_prompt_reads_xynapse_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".xynapse")).expect("xynapse dir");
        fs::write(root.join("XYNAPSE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".xynapse").join("settings.json"),
        ...
        let original_xynapse_home = std::env::var("XYNAPSE_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("XYNAPSE_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_xynapse_home {
            std::env::set_var("XYNAPSE_CONFIG_HOME", value);
        } else {
    ...
    #[test]
    fn renders_xynapse_code_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".xynapse")).expect("xynapse dir");
        fs::write(root.join("XYNAPSE.md"), "Project rules").expect("write XYNAPSE.md");
        fs::write(
            root.join(".xynapse").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        ...
        assert!(prompt.contains("# Xynapse instructions"));
        assert!(prompt.contains("Project rules"));
        assert!(prompt.contains("permissionMode"));
    ...
    #[test]
    fn discovers_dot_xynapse_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".xynapse")).expect("nested xynapse dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".xynapse/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/XYNAPSE.md"),
            content: "Project rules".to_string(),
        }]);
        assert!(rendered.contains("# Xynapse instructions"));
