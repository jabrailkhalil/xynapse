...
const AUTO_COMPACTION_THRESHOLD_ENV_VAR: &str = "XYNAPSE_CODE_AUTO_COMPACT_INPUT_TOKENS";
...
mod tests {
    ...
    #[test]
    fn reasoning_events_preserve_order_without_empty_blocks() {
        let (message, _, _) = super::build_assistant_message(vec![
            AssistantEvent::Thinking {
                thinking: String::new(),
                signature: None,
            },
        ...
        assert!(
            matches!(&message.blocks[0], ContentBlock::Thinking { thinking, signature } if thinking == "inspect" && signature.as_deref() == Some("fixture-signature"))
        );
        assert!(matches!(&message.blocks[1], ContentBlock::ToolUse { id, .. } if id == "t1"));
    }
}
