...
/// Resolve a POSIX-compatible shell on every supported desktop platform.
/// Windows does not ship `sh`, but the Git installation required by the
/// project includes Git Bash. An explicit override remains available for
/// portable and enterprise installations.
pub(crate) fn default_shell_program() -> std::path::PathBuf {
    #[cfg(windows)]
    {
        if let Some(configured) = env::var_os("XYNAPSE_BASH") {
