...
/// Write current worker state to `.xynapse/worker-state.json` under the worker's cwd.
/// This is the file-based observability surface: external observers and orchestrators
...
struct StateSnapshot<'a> {
    ...
    updated_at: u64,
    /// Seconds since last state transition. Xynapse uses this to detect
    /// stalled workers without computing epoch deltas.
    seconds_since_update: u64,
}

fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".xynapse");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".xynapse").join("worker-state.json");
        assert!(
            state_path.exists(),
