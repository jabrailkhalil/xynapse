...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        ...
        vec![
            ...
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".xynapse").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".xynapse").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("XYNAPSE_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("CLAW_CONFIG_HOME").map(PathBuf::from))
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".xynapse")))
        .unwrap_or_else(|| PathBuf::from(".xynapse"))
}
...
fn parse_optional_hooks_config_object(
    ...
    let parsed = parse_hooks_object_partial(hooks, context);
    // Keep Xynapse's fail-closed behavior for malformed supported hooks.
    if let Some(invalid) = parsed
        .invalid_hooks()
        .iter()
        .find(|entry| entry.kind != "unknown_hook_event")
    {
        return Err(ConfigError::Parse(invalid.reason.clone()));
    }
    Ok(parsed)
}
...
fn parse_mcp_server_config(
    ...
    match server_type {
        ...
        "xynapseai-proxy" => Ok(McpServerConfig::ManagedProxy(McpManagedProxyServerConfig {
            url: expect_string(object, "url", context)?.to_string(),
...
mod tests {
    ...
    #[test]
    fn loads_and_merges_xynapse_code_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        fs::create_dir_all(cwd.join(".xynapse")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
        ...
        fs::write(
            cwd.join(".xynapse").join("settings.local.json"),
    ...
    fn parses_provider_fallbacks_chain_with_primary_and_ordered_fallbacks() {
        ...
        assert_eq!(chain.primary(), Some("xynapse-opus-4-6"));
        assert_eq!(
            chain.fallbacks(),
    ...
    fn parses_user_defined_model_aliases_from_settings() {
        ...
        fs::write(
            home.join("settings.json"),
            r#"{"aliases":{"fast":"xynapse-haiku-4-5-20251213","smart":"xynapse-opus-4-6"}}"#,
        )
        .expect("write user settings");
        fs::write(
            cwd.join(".xynapse").join("settings.local.json"),
            r#"{"aliases":{"smart":"xynapse-sonnet-4-6","cheap":"grok-3-mini"}}"#,
        )
        .expect("write local settings");
        ...
        assert_eq!(
            ...
            Some("xynapse-haiku-4-5-20251213")
        );
        assert_eq!(
            aliases.get("smart").map(String::as_str),
            Some("xynapse-sonnet-4-6")
        );
        assert_eq!(
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        let project_settings = cwd.join(".xynapse").join("settings.json");
        fs::create_dir_all(cwd.join(".xynapse")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
    ...
    fn validates_unknown_top_level_keys_with_line_and_field_name() {
        ...
        let loaded = ConfigLoader::new(&cwd, &home)
            .load()
            .expect("unknown fields should not block loading");
        let source = fs::read_to_string(&user_settings).expect("synthetic config");
        let diagnostics =
            crate::config_validate::validate_config_file(&loaded.merged, &source, &user_settings);
        assert!(diagnostics.errors.is_empty());
        assert_eq!(diagnostics.warnings.len(), 1);
        let rendered = crate::config_validate::format_diagnostics(&diagnostics);
        assert!(
    ...
    #[test]
    fn warns_about_unsupported_legacy_tool_settings() {
        // given
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".claw");
        ...
        assert!(
            ...
            "warning should call out the unknown field, got: {rendered}"
        );
        // allowedTools is an unknown key; validator should name it in the warning
        assert!(
            rendered.contains("allowedTools"),
            "warning should name the offending field, got: {rendered}"
        );
