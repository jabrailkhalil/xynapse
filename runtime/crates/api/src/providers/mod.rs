        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ProviderKind::Anthropic => match *alias {
                    "opus" => "xynapse-opus-4-6",
                    "sonnet" => "xynapse-sonnet-4-6",
                    "haiku" => "xynapse-haiku-4-5-20251213",
                    _ => trimmed,
                },
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    if canonical.starts_with("xynapse") {
    ...
    if canonical.starts_with("yandex/") || canonical.starts_with("gpt://") {
        return Some(ProviderMetadata {
            provider: ProviderKind::OpenAi,
            auth_env: "YANDEX_API_KEY",
            base_url_env: "YANDEX_BASE_URL",
            default_base_url: openai_compat::DEFAULT_YANDEX_BASE_URL,
        });
    }
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match canonical.as_str() {
        "xynapse-opus-4-6" => Some(ModelTokenLimit {
            max_output_tokens: 32_000,
            context_window_tokens: 200_000,
        }),
        "xynapse-sonnet-4-6" | "xynapse-haiku-4-5-20251213" => Some(ModelTokenLimit {
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "YANDEX_API_KEY",
        "Yandex Cloud",
        "prefix your model name with `yandex/` and use a `gpt://folder/model` model URI so prefix routing selects the Yandex OpenAI-compatible backend",
    ),
];
...
mod tests {
    ...
    fn detects_provider_from_model_name_first() {
        ...
        assert_eq!(
            detect_provider_kind("xynapse-sonnet-4-6"),
            ProviderKind::Anthropic
        );
    }
    ...
    fn plugin_config_max_output_tokens_overrides_model_default() {
        ...
        let effective = max_tokens_for_model_with_override("xynapse-opus-4-6", plugin_override);

        // then
        assert_eq!(plugin_override, Some(12345));
        assert_eq!(effective, 12345);
        assert_ne!(effective, max_tokens_for_model("xynapse-opus-4-6"));
    ...
    fn max_tokens_for_model_with_override_falls_back_when_plugin_unset() {
        ...
        // then
        assert_eq!(effective, max_tokens_for_model("xynapse-opus-4-6"));
    ...
    fn returns_context_window_metadata_for_supported_models() {
        assert_eq!(
            model_token_limit("xynapse-sonnet-4-6")
                .expect("xynapse-sonnet-4-6 should be registered")
    ...
    fn preflight_blocks_requests_that_exceed_the_model_context_window() {
        ...
        match error {
            ApiError::ContextWindowExceeded {
                ...
                assert_eq!(model, "xynapse-sonnet-4-6");
