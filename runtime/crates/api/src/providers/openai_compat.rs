...
pub const DEFAULT_YANDEX_BASE_URL: &str = "https://llm.api.cloud.yandex.net/v1";
...
const YANDEX_ENV_VARS: &[&str] = &["YANDEX_API_KEY"];
...
impl OpenAiCompatConfig {
    ...
    /// It uses the `OpenAI` chat-completions body shape and accepts the
    /// folder id through `OpenAI-Project` when available.
    #[must_use]
    pub const fn yandex() -> Self {
        Self {
...
impl OpenAiCompatClient {
    ...
    async fn send_raw_request(
        ...
        let request_builder = if self.config.provider_name == "Yandex Cloud" {
            let builder = request_builder.bearer_auth(&self.api_key);
            match std::env::var("YANDEX_FOLDER_ID")
                .ok()
                .map(|value| value.trim().to_string())
                .filter(|value| !value.is_empty())
            {
                Some(folder_id) => builder.header("OpenAI-Project", folder_id),
                None => builder,
            }
        } else {
...
pub fn model_requires_reasoning_content_in_history(model: &str) -> bool {
    let routed = strip_routing_prefix(model);
    let canonical = if let Some(uri) = routed.strip_prefix("gpt://") {
        uri.split('/').nth(1).unwrap_or("")
    } else {
        routed.rsplit('/').next().unwrap_or(routed)
    };
    canonical.to_ascii_lowercase().starts_with("deepseek-v4")
}
...
fn strip_routing_prefix(model: &str) -> &str {
    if let Some(pos) = model.find('/') {
        ...
        if matches!(
            prefix,
            "openai" | "xai" | "grok" | "qwen" | "kimi" | "yandex"
        ) {
            &model[pos + 1..]
        } else {
            model
...
mod tests {
    ...
    fn grok_3_mini_is_reasoning_model() {
        ...
        assert!(!is_reasoning_model("xynapse-sonnet-4-6"));
    }
    ...
    fn model_rejects_is_error_field_detects_kimi_models() {
        ...
        assert!(!super::model_rejects_is_error_field("xynapse-sonnet-4-6"));
    ...
    fn translate_message_includes_is_error_for_non_kimi_models() {
        ...
        // Test with xynapse model (should include is_error)
        let translated3 = super::translate_message(&message, "xynapse-sonnet-4-6");
        assert!(
            translated3[0].get("is_error").is_some(),
            "xynapse should include is_error field"
        );
    }
    ...
    #[test]
    fn reasoning_history_preserves_yandex_uri_and_tool_pair() {
        for model in [
            "deepseek-v4-pro",
            "openai/deepseek-v4-flash",
            "yandex/gpt://audit-folder/deepseek-v4-flash/latest",
            "gpt://audit-folder/deepseek-v4-pro/latest",
        ] {
            let payload = build_chat_completion_request(
                &MessageRequest {
                    model: model.into(),
            ...
            assert_eq!(payload["model"], json!(super::strip_routing_prefix(model)));
            assert_eq!(
                payload["messages"][0]["reasoning_content"],
                "Inspect fixture"
            );
            assert_eq!(payload["messages"][0]["tool_calls"][0]["id"], "t1");
            assert_eq!(payload["messages"][1]["tool_call_id"], "t1");
            assert!(!payload.to_string().contains("synthetic-signature"));
        }
    }

    #[test]
    fn reasoning_history_does_not_add_unsupported_fields_to_other_models() {
        let message = InputMessage {
            role: "assistant".into(),
            content: vec![
                InputContentBlock::Thinking {
                    thinking: "Internal fixture".into(),
                    signature: None,
                },
                InputContentBlock::Text {
                    text: "Answer".into(),
        ...
        for model in [
            "qwen3:latest",
            "gpt-4o",
            "yandex/gpt://audit-folder/qwen3/latest",
        ] {
            let result = super::translate_message(&message, model);
            assert_eq!(
                result,
    ...
    #[test]
    fn reasoning_stream_closes_before_tool_use_and_keeps_arguments() {
        for field in ["reasoning_content", "reasoning", "thinking"] {
            let mut state = super::StreamState::new("deepseek-v4-flash".into());
            let value = if field == "thinking" {
            ...
            delta[field] = value;
            let first = state
                .ingest_chunk(serde_json::from_value(json!({"choices":[{"delta":delta}]})).unwrap())
                .unwrap();
            assert!(
                matches!(&first[2], crate::StreamEvent::ContentBlockDelta(event) if matches!(&event.delta, crate::ContentBlockDelta::ThinkingDelta { thinking } if thinking == "inspect"))
            );
            let next = state.ingest_chunk(serde_json::from_value(json!({"choices":[{"delta":{"tool_calls":[{"index":0,"id":"t1","function":{"name":"read","arguments":"{\"path\":\"fixture\"}"}}]},"finish_reason":"tool_calls"}]})).unwrap()).unwrap();
            assert!(
                matches!(&next[0], crate::StreamEvent::ContentBlockStop(event) if event.index == 0)
            );
            assert!(
                matches!(&next[1], crate::StreamEvent::ContentBlockStart(event) if event.index > 0 && matches!(&event.content_block, crate::OutputContentBlock::ToolUse { id, .. } if id == "t1"))
            );
            assert!(next.iter().any(|event| matches!(event, crate::StreamEvent::ContentBlockDelta(event) if matches!(&event.delta, crate::ContentBlockDelta::InputJsonDelta { partial_json } if partial_json == "{\"path\":\"fixture\"}"))));
            let end = state.finish().unwrap();
            assert!(end
                .iter()
                .any(|event| matches!(event, crate::StreamEvent::MessageStop(_))));
