...
fn base_cache_root() -> PathBuf {
    if let Some(cache_home) = std::env::var_os("XYNAPSE_CACHE_HOME") {
        return PathBuf::from(cache_home).join("prompt-cache");
    }
    if let Some(config_home) = std::env::var_os("XYNAPSE_CONFIG_HOME") {
    ...
    std::env::temp_dir().join("xynapse-prompt-cache")
}
